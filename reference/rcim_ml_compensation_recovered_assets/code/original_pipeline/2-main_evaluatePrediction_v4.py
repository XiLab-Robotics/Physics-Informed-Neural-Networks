...
def exportTableForPaper(df, error, amplPhase, outputFile):
    ...
    new_order_cols = df_data.columns[-1:].to_list() + df_data.columns[:-2].to_list()
...
def main():
    ...
    inputPath = 'output_prediction/instV3.8_Fw_allFreq_def/'
    ...
    for f in files:
        ...
        pd.DataFrame(error_tot).to_csv('evaluation/V3.9/details/dfOutTot_evaluation_details_V12'  + '_'
        ...
        outputFileSummary = 'evaluation/V3.9/dfOutTot_evaluation_V12'+ str(f.split('/')[2])+ '.csv'
        ...
        if f == files[-1]:
            for e in ['_MSE','_RMSE','_MAE','_MAPE']:
                for a in ['ampl','phase']:
                    ...
                    #     exportTableForPaper(finalOut,e,a,'evaluation/V3.9/V12_perPaper/trainDim_'
                    #                         +str(trainDim)+'_testDim_'+str(testDim))
                    # else:
                    exportTableForPaper(finalOut, e, a, 'evaluation/V3.9/')
