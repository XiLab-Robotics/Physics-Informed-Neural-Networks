...
class Instance:
    ...
    def predicted_TE(self, filename, mode, show, data, FwBw):
        ...
        if FwBw == 'Fw':
            ...
            # Sum the harmonics together to get the reconstructed signal
            reconstructed_signal_previsto = np.sum(harmonics, axis=0)
            harmonics_fft_filterd = [self.fft_y_Fw_filtered_ampl[i]*
                                     np.cos(2 *n* np.pi * k / len(self.x_Fw)+self.fft_y_Fw_filtered_phase[i]) for i,k in enumerate(self.fft_listFreq)]
            reconstructed_signal_fft_filtered = np.sum(harmonics_fft_filterd, axis=0)
        elif FwBw == 'Bw':
            ...
            # Calculate the harmonics of the selected frequencies using cosine
            harmonics = [ampl[i] * np.cos(2 * n * np.pi * k / len(self.x_Bw) + phase[i]) for i, k in
                         enumerate(self.fft_listFreq)]
            # Sum the harmonics together to get the reconstructed signal
            reconstructed_signal_previsto = np.sum(harmonics, axis=0)

            harmonics_fft_filterd = [self.fft_y_Bw_filtered_ampl[i] *
                                     np.cos(2 * n * np.pi * k / len(self.x_Bw) + self.fft_y_Bw_filtered_phase[i]) for
                                     i, k in enumerate(self.fft_listFreq)]
            reconstructed_signal_fft_filtered = np.sum(harmonics_fft_filterd, axis=0)
        else:
