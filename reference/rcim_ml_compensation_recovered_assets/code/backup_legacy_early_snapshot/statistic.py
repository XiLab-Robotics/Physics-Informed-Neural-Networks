...
class Statistics:
        ...
        for elem in self.instances:
            ...
            if len(getattr(elem, valueAttribute))>0:
                for i in range(len(getattr(elem, valueAttribute))):
                    row[valueAttribute+"_"+str(i)] = getattr(elem, valueAttribute)[i]
            errors.append(row)
        df = pd.DataFrame(errors)
        return df
    ...
    def genDfWithAmplEPhase(self):
        df_ampl = self._generateDf_differentFreq('fft_y_Fw_filtered_ampl')
        df_phase = self._generateDf_differentFreq('fft_y_Fw_filtered_phase')
