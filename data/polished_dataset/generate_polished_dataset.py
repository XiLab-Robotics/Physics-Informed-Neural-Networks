...
INPUT_PATH = r"C:\Users\ber\Measurement_Data\XiLab\2024-07-19"
OUTPUT_PATH = r"C:\Users\ber\Measurement_Data\XiLab\2024-07-19\csv_python"
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in csv.reader(handle, delimiter=delimiter):
            if len(row) < 11:
                continue
            try:
                data["theta_enc_deg"].append(number(row[THETA_ENC_DEG]))
                data["q_enc_deg"].append(number(row[Q_ENC_DEG]))
                data["tau_load_nm"].append(number(row[TAU_LOAD_NM]))
                data["valid_fw"].append(number(row[VALID_FW]))
                data["valid_bw"].append(number(row[VALID_BW]))
                data["temp_deg_c"].append(number(row[TEMP_DEG_C]))
                data["q_abs_deg"].append(number(row[Q_ABS_DEG]))
            except ValueError:
                continue
...
def is_generated_folder(name):
    name = name.lower()
    return name == "processed" or name.startswith("csv")
...
def main():
    ...
    for path, conditions in unique_condition_records(records):
        try:
    ...
    if skipped:
        print(f"Skipped {skipped} input files")
