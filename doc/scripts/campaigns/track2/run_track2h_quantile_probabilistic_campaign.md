...
```powershell
.\scripts\campaigns\track2\run_track2h_quantile_probabilistic_campaign.ps1 -PreflightOnly -RunOneBatchValidation
```

Local enqueue-only verification:

```powershell
.\scripts\campaigns\track2\run_track2h_quantile_probabilistic_campaign.ps1 -EnqueueOnly
```
