...
Dedicated campaign launchers should expose this same path through a `-Remote`
switch. The dedicated wrapper remains the operator-facing command, while the
shared infrastructure launcher owns the source sync, remote run, artifact sync,
and local status-file updates.
