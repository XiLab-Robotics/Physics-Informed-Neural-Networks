# RCIM Model-Bank Reproduction Forward Last Three Open Cells Campaign Launcher

## Overview

This launcher executes the prepared final targeted forward-only `RCIM Model-Bank Reproduction`
original-dataset residual repair campaign.

The script is stored in:

- `scripts/campaigns/track_1/exact_paper/run_track1_forward_last_three_open_cells_campaign.ps1`
...
The campaign purpose is final forward closure pressure, not another
family-bank sweep:
...
- `84` total YAML runs
- extra depth only for the last stubborn single-surface residual pair
...
```powershell
conda run -n pinns_env python scripts/campaigns/track_1/exact_paper/prepare_track1_forward_last_three_open_cells_campaign.py
```
...
- `24` retries for each standard `yellow_single_surface` pair
- `36` retries for the only `yellow_single_surface_stubborn` pair
...
```powershell
.\scripts\campaigns\track_1\exact_paper\run_track1_forward_last_three_open_cells_campaign.ps1 -Remote
```
