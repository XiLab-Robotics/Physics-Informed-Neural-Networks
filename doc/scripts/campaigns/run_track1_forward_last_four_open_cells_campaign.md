# RCIM Model-Bank Reproduction Forward Last Four Open Cells Campaign Launcher
...
- `scripts/campaigns/track_1/exact_paper/run_track1_forward_last_four_open_cells_campaign.ps1`
...
- `84` total YAML runs
- extra depth only for the last multi-surface residual pair
...
```powershell
conda run -n pinns_env python scripts/campaigns/track_1/exact_paper/prepare_track1_forward_last_four_open_cells_campaign.py
```
...
- `24` retries for each `yellow_single_surface` pair
- `36` retries for the only `yellow_multi_surface` pair
...
```powershell
.\scripts\campaigns\track_1\exact_paper\run_track1_forward_last_four_open_cells_campaign.ps1 -Remote
```
