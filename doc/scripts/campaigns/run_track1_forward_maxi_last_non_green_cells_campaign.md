...
- `270` total YAML runs
- tiered escalation depth based on the remaining yellow and red amplitude
  pressure
...
```powershell
conda run -n standard_ml_codex_env python scripts/campaigns/track1/exact_paper/prepare_track1_forward_maxi_last_non_green_cells_campaign.py
```
...
The retry policy is intentionally deeper than the previous `108`-run wave:

- `24` retries for each `yellow_single_surface` pair
- `36` retries for each `yellow_multi_surface` pair
- `63` retries for each `red_multi_surface` pair
