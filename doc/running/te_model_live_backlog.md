...
| Shape-gate metric loss pilot | Evaluate whether the shape-gated reranker metrics can become normalized auxiliary training losses or checkpoint-selection monitors for a small polished `periodic_gru_sequence`-anchored pilot. | first `polished_dataset` setpoint `Fw` pilot completed and evaluated as a forward-only Track 2 diagnostic; it failed the shape gate and is not promoted. Continue only after auditing the full-curve playback contract, offset handling, and sequence-window reconstruction gap |
...
| Shape-gate loss pilot | Test whether calibrated shape-gate evidence can improve training or checkpoint selection. | one-run `polished_dataset` `setpoints` `Fw` pilot closed; scalar error improved versus some baselines, but Track 2 full-curve playback produced a strong offset-dominated failure, so the branch remains diagnostic. Full promotion still requires `simplified_setpoints`, `polished_setpoints`, and `polished_actual_values`, each across `global`, `Fw`, and `Bw` |
