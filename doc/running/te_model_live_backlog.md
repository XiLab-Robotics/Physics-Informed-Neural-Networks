...
| Shape-gate loss pilot | Test whether calibrated shape-gate evidence can improve training or checkpoint selection. | plan with `polished_dataset` `setpoints` `Fw` as the primary pilot surface; full promotion requires `simplified_setpoints`, `polished_setpoints`, and `polished_actual_values`, each across `global`, `Fw`, and `Bw` |
