...
| `Wave 5.2` dataset-aware MMT / PINN-guided branch | Split first-PINN planning into clean-polished modeling, simplified noise-aware diagnostics, and paired dirty-to-clean or transfer tests. | `Wave 5.2A` MMT diagnostic, parameter inventory, and first bounded paired-dataset diagnostic are generated; full-matrix paired diagnostics, `Wave 5.2B`, and `Wave 5.2C` gates remain open |
