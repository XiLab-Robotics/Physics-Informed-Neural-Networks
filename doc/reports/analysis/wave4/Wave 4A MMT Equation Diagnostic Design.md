# Wave 4A MMT Equation Diagnostic Design

## Purpose

`Wave 4A` turns the repository-owned `MMT_TEModeling` equation reproduction
into a diagnostic tool for Track 2 curves. It does not train a model. Its goal
...
| Inputs | Track 2 angle grid, direction, speed, torque, temperature, reducer geometry assumptions, and candidate equivalent-error vectors. |
...
| Comparison targets | Accepted Track 2 leaders, Track 2G, Track 2H, Wave 2B, Wave 2C, and Wave 3 designs. |
...
5. Export per-curve MMT diagnostics and Track 2 metric joins.
...
Promote to `Wave 4B` only if at least one MMT intermediate term or calibrated
analytical residual aligns with Track 2 offset, harmonic, or condition-regime
failure modes.
