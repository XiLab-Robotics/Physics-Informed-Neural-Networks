# Track 2 Report Grouped Source Tables

## Overview

This technical document formalizes a readability refinement for the canonical
`Track 2` report after the comparison matrix was extended to include:

- accepted `Track 1` reference banks;
...
tables by source family: original, retuned, Track 1, and Wave 1.
...
Update the Track 2 Markdown report generator so the canonical report contains
...
1. Extend the Track 2 report builder with a reusable grouped-table helper keyed
...
3. Replace the flat forward and backward comparison sections with grouped
   subsections for original, retuned, Track 1, and Wave 1 candidates.
4. Regenerate the canonical Track 2 report and latest validation report from
...
7. Run commit preflight, then commit the approved Track 2 matrix extension and
