# Track 2 Incremental Visual Artifact Sync

## Overview

This technical note plans the cleanup of the `Track 2` refresh launcher and
visual artifact generation after the `Wave 2B` harmonic-temporal refresh showed
...
Keep the default `Track 2` refresh focused on new candidate artifacts:
...
1. Add a configuration switch that defaults grouped report plots to the current
   ...
   plot regeneration.
2. Update the `Track 2` comparison runner so merged baseline summaries retain
   existing metrics while only the current run contributes new grouped plot
   paths by default.
3. Update the Wave 2B operator launcher so remote artifact sync pulls the
