...
Plan the `Wave 2.3` follow-up to the completed `Wave 2.2` harmonic-temporal
hybrid campaign.

`Wave 2.2` verified that sparse `RCIM` harmonic features materially improve the
...
Each model should preserve the existing `Wave 2.1` sequence-window contract and
...
because the Wave 2.2 leaderboard showed the recurrent families as the useful
...
- `scripts/models/periodic_temporal_sequence_network.py`
  Existing sequence input-context pattern and Wave 2.2 harmonic-temporal
...
11. After completion, perform normal campaign closeout before proposing any
    optional `TE Curve Verification Pipeline` refresh.
