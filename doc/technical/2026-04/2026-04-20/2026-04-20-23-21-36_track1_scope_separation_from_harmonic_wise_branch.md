...
- `RCIM Model-Bank Reproduction` is considered complete only when the repository has the expected
...
- the harmonic-wise branch should be postponed into a separate follow-up track,
  named either `RCIM Harmonic-Wise Follow-Up` or `TE Curve Verification Pipeline`, rather than remaining part of
  canonical `RCIM Model-Bank Reproduction` status.
...
- canonical `RCIM Model-Bank Reproduction` progress is measured only through the four full-matrix
...
- `Table 6` may remain in the benchmark as a historical or secondary support
  surface only if it is no longer treated as a canonical `RCIM Model-Bank Reproduction` closure
  gate;
...
- `RCIM Model-Bank Reproduction`: exact-paper full family-bank replication, with `19` models for
  each of `10` families;
- `RCIM Harmonic-Wise Follow-Up`: postponed harmonic-wise branch used for later analysis,
...
be labeled `TE Curve Verification Pipeline`, but the touched documents should use one label
...
1. Create and register this technical document for the `RCIM Model-Bank Reproduction` scope repair.
2. Update `RCIM Paper Reference Benchmark.md` so canonical `RCIM Model-Bank Reproduction` status is
...
3. Remove or downgrade any wording that treats `Table 6` harmonic closure as a
   primary `RCIM Model-Bank Reproduction` status gate.
4. Update `Training Results Master Summary.md` so the active and planned branch
   wording no longer presents harmonic-wise work as the main `RCIM Model-Bank Reproduction` branch.
5. Rename the harmonic-wise branch consistently as a postponed follow-up track
   such as `RCIM Harmonic-Wise Follow-Up` or `TE Curve Verification Pipeline`.
