# Wave 1 Familywise Hyperparameter Optimization Campaign
...
This technical document defines the `Wave 1` follow-up direction as a
familywise best-training campaign program rather than one mixed cross-family
queue.

## Technical Approach

The `Wave 1` follow-up program should be organized as a sequence of
family-specific
...
The `Wave 1` follow-up optimization program should be split into separate
family tracks.
...
selected family has a tuned `Wave 1` follow-up champion.
...
- `doc/reports/campaign_plans/`
  ...
  created before any new familywise follow-up training execution.
...
- `output/training_campaigns/`
  Artifact root for the resulting familywise follow-up campaign outputs.
...
3. After approval, create the first familywise follow-up planning report in
