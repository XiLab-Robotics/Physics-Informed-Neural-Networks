...
the Wave 3 model or campaign YAMLs:
...
The fix should be applied to the shared remote launcher, not to the Wave 3
campaign package:
...
- leave the Wave 3 queue YAMLs unchanged;
...
- `doc/running/active_training_campaign.yaml` currently records the failed
  Wave 3 campaign as `running` and should be restored to a relaunchable
...
3. Restore `doc/running/active_training_campaign.yaml` from `running` to
   `prepared` for the same Wave 3 campaign, preserving the launch commands and
   protected-file list.
4. Run PowerShell parser validation on the shared remote launcher.
5. Run Markdown QA on touched authored Markdown.
6. Run the Wave 3 launcher `-PreflightOnly` locally.
