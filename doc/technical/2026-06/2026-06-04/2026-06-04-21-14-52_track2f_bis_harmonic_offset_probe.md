...
This technical document prepares the next Wave 3.1 follow-up branch after the
completed `sequential_residual_offset_probe` campaign and the clean-baseline
documentation update.

The completed Wave 3.1 branch is useful as a non-harmonic control, but it does
...
sequence model is used, only from already available causal history. TE Curve Verification Pipeline
curves remain an offline validation and promotion surface, not a future input
to the deployed model.

## Technical Approach

Wave 3.2 should be implemented as a compact, direction-parallel probe with
two model branches:

- clean control branch: retain a Wave 3.1-like
...
harmonic banks can remain ablations, but the previous Wave 2.3 evidence says
...
diagnostics and later evaluate with TE Curve Verification Pipeline curve-first metrics. A later
...
1. Create a preliminary Wave 3.2 campaign planning report under
...
6. Materialize queue YAMLs for `global`, `Fw`, and `Bw` for both:
   the clean Wave 3.1-like baseline and the harmonic-offset branch.
...
11. Keep full TE curve verification as a separate operator-launched refresh
