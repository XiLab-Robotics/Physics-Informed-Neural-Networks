# Track 2F-Bis Official Track 2 Refresh Preparation

## Overview

This technical document plans the operator-launched official `Track 2`
...
official Track 2 direction-aware matrix, visual overlays, and companion PDF
reports.
...
Prepare a dedicated operator-facing Track 2F-bis verification launcher without
running the heavy matrix inside Codex. The launcher will update or wrap the
official Track 2 matrix workflow so the six registry-backed candidates above
are included with correct direction scopes:

- `global`: evaluate against forward and backward Track 2 curves;
...
comparison with a Track 2F-bis-specific output suffix. The launcher must also
...
artifacts will be inspected and accepted through the standard Track 2 refresh
workflow:

- confirm the candidate inventory and matrix count include the six new
  Track 2F-bis candidates;
...
2B, or prior Track 2F baseline changes. The closeout metrics alone do not
justify promotion.
...
- future launcher under `scripts/campaigns/track2/`
- future launcher note under `doc/scripts/campaigns/track2/`
...
2. Add the six Track 2F-bis candidate entries to the compact Track 2 matrix
   configuration with correct `global`, `Fw`, and `Bw` direction scopes.
3. Create a dedicated PowerShell launcher for the Track 2F-bis verification
...
7. After completion, inspect the generated matrix artifacts and confirm the
   six Track 2F-bis candidates are present in the directional report.
8. Regenerate the best-model collage and multi-model curve comparison reports
   for the dated refresh bundle.
9. Create the dated official Track 2 model-verification report that records
