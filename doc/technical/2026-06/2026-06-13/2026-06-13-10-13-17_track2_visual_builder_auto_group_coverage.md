# TE Curve Verification Pipeline Visual Builder Auto Group Coverage

## Overview

`TE Curve Verification Pipeline` refresh launchers already run the matrix, collage report, overlay
...
This document plans a tooling fix for future `TE Curve Verification Pipeline` refreshes: visual
reports should discover current registry-model groups from the same matrix
configuration used by the official `TE Curve Verification Pipeline` run, and the launcher should fail
early if visual coverage is incomplete.

## Technical Approach

The current `Wave 4.2` quantile/probabilistic refresh showed the failure mode:
...
`Wave 4.1` robust-loss groups. The fix should remove that manual update point.
...
latent-state, or later multi-head `TE Curve Verification Pipeline` refresh can run once: when the
...
- `scripts/reports/analysis/build_track2_best_model_collage_report.py`
  ...
  construction for prior TE Curve Verification Pipeline branches.
...
5. Wire that validation into the Wave 4.2 quantile/probabilistic launcher after
