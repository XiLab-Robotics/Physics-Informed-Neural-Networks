# Wave 3 Track 2 PDF Asset Path Repair

## Overview

The completed `Wave 3` harmonic-prior residual `Track 2` refresh generated the
dated visual report bundle under
`doc/reports/analysis/track2/*/[2026-06-15]/`. The real styled PDF export did
not render several `Wave 3` collage images even though the referenced `PNG`
...
hitting very long asset paths after the report builder copied `Wave 3` images
...
the heavy `Track 2` matrix. Patch the visual report builders so automatically
...
The expected durable outcome is that future `Wave 3` or similarly named
...
3. Regenerate the dated `Wave 3` visual Markdown bundles for
...
5. Raster-validate representative PDF pages, including the previously broken
   `Wave 3` collage pages, and confirm the images render in the real PDF.
