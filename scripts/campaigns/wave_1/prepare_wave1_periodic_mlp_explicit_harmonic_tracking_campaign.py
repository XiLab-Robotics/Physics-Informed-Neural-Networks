"""Prepare the Wave 1 periodic MLP explicit harmonic tracking campaign."""
...
CAMPAIGN_ROOT = Path("config/training/wave1_periodic_mlp_explicit_harmonic_tracking/campaigns/2026-05-20_wave1_periodic_mlp_explicit_harmonic_tracking_campaign")
...
PLANNING_REPORT_PATH = Path("doc/reports/campaign_plans/wave_1/2026-05-20-22-42-49_wave1_periodic_mlp_explicit_harmonic_tracking_campaign_plan_report.md")
TECHNICAL_DOCUMENT_PATH = Path("doc/technical/2026-05/2026-05-20/2026-05-20-22-34-11_periodic_mlp_explicit_harmonic_basis.md")
LAUNCHER_PATH = Path("scripts/campaigns/wave_1/run_wave1_periodic_mlp_explicit_harmonic_tracking_campaign.ps1")
LAUNCHER_NOTE_PATH = Path("doc/scripts/campaigns/run_wave1_periodic_mlp_explicit_harmonic_tracking_campaign.md")
ACTIVE_CAMPAIGN_STATE_PATH = Path("doc/running/active_training_campaign.yaml")
CAMPAIGN_OUTPUT_DIRECTORY = Path("output/training_campaigns/wave1/periodic_mlp_explicit_harmonic_tracking") / CAMPAIGN_NAME

HARMONIC_BANK_DICTIONARY = {
    "rcim_sparse": {
        "harmonic_order": 240,
        "harmonic_index_list": [0, 1, 3, 39, 40, 78, 81, 156, 162, 240],
        "description": "RCIM selected sparse periodic-feature bank",
    },
    "dense240": {
        "harmonic_order": 240,
        "harmonic_index_list": list(range(0, 241)),
        "description": "dense paper-maximum periodic-feature bank 0..240",
    },
    "dense360": {
        "harmonic_order": 360,
        "harmonic_index_list": list(range(0, 361)),
        "description": "extended dense periodic-feature bank 0..360",
    },
}

SCOPE_SOURCE_FILE_DICTIONARY = {
    "global": "periodic_mlp.yaml",
    "fw": "periodic_mlp_fw.yaml",
    "bw": "periodic_mlp_bw.yaml",
}
...
def build_run_name(scope_key: str, bank_key: str) -> str:

    """Build a compact campaign run name."""

    scope_label = SCOPE_LABEL_DICTIONARY[scope_key]
    return f"te_periodic_mlp_{bank_key}_tracking_{scope_label}"
...
def build_campaign_config(
    ...
    """Create one periodic MLP explicit harmonic tracking configuration."""

    campaign_config = copy.deepcopy(source_config)
    run_name = build_run_name(scope_key, bank_key)
    campaign_config_id = f"periodic_mlp_{scope_key}_{bank_key}"
    ...
    metadata_config["phase_name"] = "wave1_periodic_mlp_explicit_harmonic_tracking"
    ...
    metadata_config["notes"] = (
        f"Wave 1 periodic MLP explicit harmonic tracking follow-up | {bank_description} | "
        f"scope={scope_key}. Baseline comparison uses existing Wave 1 periodic_mlp best runs."
    )
    metadata_config.pop("output_run_name", None)
    metadata_config.pop("run_instance_id", None)

    # Update Periodic Feature Basis
...
def build_queue_config_file_name(config_index: int, scope_key: str, bank_key: str) -> str:

    """Build one stable queue configuration filename."""

    return f"{config_index:02d}_periodic_mlp_{scope_key}_{bank_key}.yaml"
...
def write_launcher_note() -> None:
    ...
    launcher_note_text = f"""# Wave 1 Periodic MLP Explicit Harmonic Tracking Campaign Launcher
...
def write_campaign_readme(queue_file_name_list: list[str]) -> None:
    ...
    readme_text = f"""# Wave 1 Periodic MLP Explicit Harmonic Tracking Campaign
...
def write_active_campaign_state(queue_config_relative_path_list: list[str], protected_file_relative_path_list: list[str]) -> None:
    ...
    active_campaign_state = {
        ...
        "prepared_at": "2026-05-20T22:42:49+02:00",
...
def prepare_campaign_package() -> None:
    ...
    """Prepare the complete periodic MLP explicit harmonic tracking package."""
    ...
    for scope_key, source_file_name in SCOPE_SOURCE_FILE_DICTIONARY.items():
        ...
        for bank_key, bank_config in HARMONIC_BANK_DICTIONARY.items():
            ...
            config_file_name = build_queue_config_file_name(config_index, scope_key, bank_key)
    ...
    print("[INFO] Launch command | .\\scripts\\campaigns\\wave1\\run_wave1_periodic_mlp_explicit_harmonic_tracking_campaign.ps1")
