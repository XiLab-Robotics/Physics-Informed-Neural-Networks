...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --dry-run)
            ...
            ;;
        --help|-h)
            echo "Usage: bash scripts/campaigns/wave_1/run_wave1_directional_optuna_recovery_micro_campaign.sh [--conda-environment-name NAME] [--python-executable COMMAND] [--gpu-id ID] [--dry-run]"
            exit 0
            ;;
        *)
