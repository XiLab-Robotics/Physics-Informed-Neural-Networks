...
if (`$null -eq `$manifestPath) {
        throw 'No remote Wave 4.4 artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Wave 4.4 artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Wave 4.4 refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
...
"@
    ...
    Write-StatusLine "DONE" "Remote Wave 4.4 refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Wave 4.4 verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            $ReportDate,
            "--refresh-label",
            "Wave 4.4 latent-state hysteresis refresh",
...
Write-StatusLine "DONE" "Wave 4.4 verification refresh completed"
