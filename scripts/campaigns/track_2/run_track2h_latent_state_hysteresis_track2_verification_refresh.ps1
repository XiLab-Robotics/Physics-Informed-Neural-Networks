param(
    ...
    [string]$OutputSuffix = "track2h_latent_state_hysteresis_track2_refresh_2026_06_18",
    [string]$ReportDate = "2026-06-18",
...
$newCandidateCampaignResultPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\track2h_latent_state_hysteresis_registry"
...
$sourceSyncPathList = @(
    ...
    "output\registries\families\track2h_latent_state_hysteresis_gru_offset_residual_global",
    "output\registries\families\track2h_latent_state_hysteresis_gru_offset_residual_fw",
    "output\registries\families\track2h_latent_state_hysteresis_gru_offset_residual_bw",
    "output\registries\families\track2h_latent_state_hysteresis_causal_tcn_offset_residual_global",
    "output\registries\families\track2h_latent_state_hysteresis_causal_tcn_offset_residual_fw",
    "output\registries\families\track2h_latent_state_hysteresis_causal_tcn_offset_residual_bw",
    "output\training_runs\track2h_latent_state_hysteresis_gru_offset_residual_global",
    "output\training_runs\track2h_latent_state_hysteresis_gru_offset_residual_fw",
    "output\training_runs\track2h_latent_state_hysteresis_gru_offset_residual_bw",
    "output\training_runs\track2h_latent_state_hysteresis_causal_tcn_offset_residual_global",
    "output\training_runs\track2h_latent_state_hysteresis_causal_tcn_offset_residual_fw",
    "output\training_runs\track2h_latent_state_hysteresis_causal_tcn_offset_residual_bw",
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("track2h_l_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("track2h_l_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("track2h_l_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2h_l_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("track2h_l_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2h_l_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
...
if (`$null -eq `$manifestPath) {
        throw 'No remote Track 2H-L artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Track 2H-L artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Track 2H-L refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
    Invoke-RemoteSourceSync

    $remoteScriptPath = "scripts\campaigns\track_2\run_track2h_latent_state_hysteresis_track2_verification_refresh.ps1"
    $remoteCommand = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
...
"@
    ...
    Write-StatusLine "DONE" "Remote Track 2H-L refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Track 2H-L verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            $ReportDate,
            "--refresh-label",
            "Track 2H-L latent-state hysteresis refresh",
            "--candidate-source-label",
            "track2h_latent_state_hysteresis_registry",
            "--decision",
            "pending operator review; evaluate with multi-index curve-first evidence before promotion",
            "--next-step",
            "Use the latent-state hysteresis evidence to judge whether causal hidden-state behavior improves offset continuity or shape fidelity beyond the existing dispersion-aware branches.",
            "--output-report-path",
            $officialReportPath,
            "--operator-log-root",
            $logRoot
...
Write-StatusLine "DONE" "Track 2H-L verification refresh completed"
