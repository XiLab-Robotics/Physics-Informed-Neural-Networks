"""Validate the prepared Wave 4.3 mixture-density heads package."""
...
def validate_queue_matrix(queue_config_list: list[dict[str, Any]]) -> None:
    ...
    """Validate the 2 by 3 Wave 5.2 series MDN queue matrix."""
    ...
    assert not missing_pair_set, f"Missing Wave 5.2 series MDN queue pairs | {sorted(missing_pair_set)}"
    assert not unexpected_pair_set, f"Unexpected Wave 5.2 series MDN queue pairs | {sorted(unexpected_pair_set)}"
...
def validate_active_campaign_state() -> None:
    ...
    assert active_state.get("status") == "prepared", "Wave 5.2 series MDN campaign state is not prepared."
    assert active_state.get("campaign_name") == CAMPAIGN_NAME, "Active state does not point at Wave 5.2 series MDN."
...
def main() -> int:
    ...
    print(
        "Wave 4.3 mixture-density heads package validated | "
