...
if (`$null -eq `$manifestPath) {
        throw 'No remote Track 2H quantile/probabilistic artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Track 2H quantile/probabilistic artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Track 2H quantile/probabilistic refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
...
"@
    ...
    Write-StatusLine "DONE" "Remote Track 2H quantile/probabilistic refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Track 2H quantile/probabilistic verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            $ReportDate,
            "--refresh-label",
            "Track 2H quantile/probabilistic refresh",
...
Write-StatusLine "DONE" "Track 2H quantile/probabilistic verification refresh completed"
