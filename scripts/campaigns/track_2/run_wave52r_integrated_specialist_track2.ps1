param(
    ...
    [string]$OutputSuffix = "wave52r_integrated_specialist_track2"
)
...
$ConfigPath = "config\paper_reimplementation\rcim_ml_compensation\reference_family_vs_feedforward\wave52r_integrated_specialist_track2_matrix.yaml"
$PreparationPath = "scripts\analysis\wave_5_2r\prepare_wave52r_integrated_specialist_track2.py"
$ValidatorPath = "scripts\analysis\wave_5_2r\validate_wave52r_integrated_specialist_track2_package.py"
...
$RemoteManifestPath = "output\analysis\wave_5_2r\integrated_specialist_track2\remote_source_path_list.txt"
...
function Invoke-LocalPreflight {
    foreach ($path in @($PreparationPath, $ValidatorPath, $MatrixRunnerPath)) { Assert-RelativePathExists $path }
    Invoke-CondaPython "integrated_specialist_package_preparation" @("-B", $PreparationPath)
    Assert-RelativePathExists $ConfigPath
    Assert-RelativePathExists $RemoteManifestPath
    Invoke-CondaPython "integrated_specialist_package_preflight" @("-B", $ValidatorPath, "--config-path", $ConfigPath, "--windows")
    Write-StatusLine "PASS" "29 candidates passed preflight; the three matrix surfaces have not run."
}
...
function Invoke-RemoteSourceSync {
    ...
    $staticPaths = @(
        ...
        "doc\scripts\campaigns\track_2\run_wave52r_integrated_specialist_track2.md"
    )
    ...
    $localArchive = Join-Path $syncRoot ("wave52r_integrated_specialist_track2_{0}.tar" -f $RunStamp)
    $remoteArchive = Join-Path $RemoteRepositoryPath ".temp\wave52r_integrated_specialist_track2_sync.tar"
    Push-Location $ProjectRoot
    try { & tar -cf $localArchive @sourcePaths; if ($LASTEXITCODE -ne 0) { throw "Source archive failed." } }
    finally { Pop-Location }
    Invoke-RemotePowerShellScript "New-Item -ItemType Directory -Force -Path '$RemoteRepositoryPath\.temp' | Out-Null" "prepare_integrated_sync"
    ...
    Invoke-RemotePowerShellScript "Set-Location -LiteralPath '$RemoteRepositoryPath'; tar -xf '$remoteArchive'; if (`$LASTEXITCODE -ne 0) { throw 'Extraction failed.' }; Remove-Item -LiteralPath '$remoteArchive' -Force" "extract_integrated_sync"
    Remove-Item -LiteralPath $localArchive -Force
}

function Invoke-RemoteArtifactSync {
    New-Item -ItemType Directory -Force -Path $LogRoot | Out-Null
    $remoteArchive = Join-Path $RemoteRepositoryPath ".temp\wave52r_integrated_specialist_track2_results_$RunStamp.zip"
    $bundleScript = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
`$bundleRoot = Join-Path `$env:TEMP 'wave52r_integrated_specialist_track2_results_$RunStamp'
...
foreach (`$source in `$sources) {
    `$repositoryPrefix = '$RemoteRepositoryPath'.TrimEnd('\') + '\'
    if (-not `$source.FullName.StartsWith(`$repositoryPrefix)) {
        throw ('Refusing to bundle a path outside the remote repository: ' + `$source.FullName)
    }
    `$relative = `$source.FullName.Substring(`$repositoryPrefix.Length)
...
"@
    Invoke-RemotePowerShellScript $bundleScript "bundle_integrated_results"
    $localArchive = Join-Path $LogRoot "remote_integrated_specialist_track2_results.zip"
    ...
    Remove-Item -LiteralPath $localArchive -Force
    Invoke-RemotePowerShellScript "Remove-Item -LiteralPath '$remoteArchive' -Force -ErrorAction SilentlyContinue" "cleanup_integrated_results"
}

if ($SyncOnly) {
    if (-not $Remote) { throw "-SyncOnly requires -Remote." }
    Write-StatusLine "STEP" "Recovering completed remote Track 2 artifacts without rerunning the matrices."
    Invoke-RemoteArtifactSync
    Write-StatusLine "DONE" "Completed remote Track 2 artifacts were synchronized locally."
    exit 0
}
...
if ($Remote) {
    ...
    $remoteLauncher = Join-Path $RemoteRepositoryPath "scripts\campaigns\track_2\run_wave52r_integrated_specialist_track2.ps1"
    $remoteFlag = if ($Run) { "-Run" } else { "-PreflightOnly" }
    Invoke-RemotePowerShellScript "& '$remoteLauncher' $remoteFlag -CondaEnvironmentName '$RemoteCondaEnvironmentName' -OutputSuffix '$OutputSuffix'" "launch_integrated_track2"
