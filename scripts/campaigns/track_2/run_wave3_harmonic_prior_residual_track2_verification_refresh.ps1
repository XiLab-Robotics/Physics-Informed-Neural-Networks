param(
    ...
    [string]$OutputSuffix = "wave3_harmonic_prior_residual_track2_refresh_2026_06_15",
    [string]$ReportDate = "2026-06-15",
...
$newCandidateCampaignResultPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\wave3_harmonic_prior_residual_registry"
...
$sourceSyncPathList = @(
    ...
    "output\registries\families\wave3_harmonic_prior_residual_pointwise_control_global",
    "output\registries\families\wave3_harmonic_prior_residual_pointwise_control_fw",
    "output\registries\families\wave3_harmonic_prior_residual_pointwise_control_bw",
    "output\registries\families\wave3_harmonic_prior_residual_smooth_l1_structured_global",
    "output\registries\families\wave3_harmonic_prior_residual_smooth_l1_structured_fw",
    "output\registries\families\wave3_harmonic_prior_residual_smooth_l1_structured_bw",
    "output\training_runs\wave3_harmonic_prior_residual_pointwise_control_global",
    "output\training_runs\wave3_harmonic_prior_residual_pointwise_control_fw",
    "output\training_runs\wave3_harmonic_prior_residual_pointwise_control_bw",
    "output\training_runs\wave3_harmonic_prior_residual_smooth_l1_structured_global",
    "output\training_runs\wave3_harmonic_prior_residual_smooth_l1_structured_fw",
    "output\training_runs\wave3_harmonic_prior_residual_smooth_l1_structured_bw",
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("wave3_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("wave3_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("wave3_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\wave3_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("wave3_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\wave3_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
