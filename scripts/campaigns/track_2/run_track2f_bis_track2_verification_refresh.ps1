param(
    ...
    [string]$OutputSuffix = "track2f_bis_harmonic_offset_probe_track2_refresh_2026_06_08",
    [string]$ReportDate = "2026-06-08",
...
$newCandidateCampaignResultPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\track2f_bis_harmonic_offset_probe_registry"
...
$sourceSyncPathList = @(
    ...
    "output\registries\families\track2f_bis_clean_sequential_residual_offset_global",
    "output\registries\families\track2f_bis_clean_sequential_residual_offset_fw",
    "output\registries\families\track2f_bis_clean_sequential_residual_offset_bw",
    "output\registries\families\track2f_bis_harmonic_residual_offset_global",
    "output\registries\families\track2f_bis_harmonic_residual_offset_fw",
    "output\registries\families\track2f_bis_harmonic_residual_offset_bw",
    "output\training_runs\track2f_bis_clean_sequential_residual_offset_global",
    "output\training_runs\track2f_bis_clean_sequential_residual_offset_fw",
    "output\training_runs\track2f_bis_clean_sequential_residual_offset_bw",
    "output\training_runs\track2f_bis_harmonic_residual_offset_global",
    "output\training_runs\track2f_bis_harmonic_residual_offset_fw",
    "output\training_runs\track2f_bis_harmonic_residual_offset_bw",
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("track2f_bis_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("track2f_bis_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("track2f_bis_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2f_bis_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("track2f_bis_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2f_bis_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
...
if (`$null -eq `$manifestPath) {
        throw 'No remote Track 2F-bis artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Track 2F-bis artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Track 2F-bis refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
    Invoke-RemoteSourceSync

    $remoteScriptPath = "scripts\campaigns\track_2\run_track2f_bis_track2_verification_refresh.ps1"
    $remoteCommand = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
...
"@
    ...
    Write-StatusLine "DONE" "Remote Track 2F-bis refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Track 2F-bis verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            $ReportDate,
            "--refresh-label",
            "Track 2F-bis harmonic-offset refresh",
            ...
            "--next-step",
            "Use the Track 2F-bis offset evidence as the baseline for later dispersion-aware and structured offset handling.",
            "--output-report-path",
            $officialReportPath,
            "--operator-log-root",
            $logRoot
...
Write-StatusLine "DONE" "Track 2F-bis verification refresh completed"
