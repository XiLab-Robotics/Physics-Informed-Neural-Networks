param(
    ...
    [string]$OutputSuffix = "track2g_curve_aware_track2_refresh_2026_06_09",
    [string]$ReportDate = "2026-06-09",
...
$newCandidateCampaignResultPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\track2g_curve_aware_training_registry"
...
$sourceSyncPathList = @(
    "scripts",
    "config",
    "doc\scripts\campaigns\track_2",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_pointwise_control_global",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_pointwise_control_fw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_pointwise_control_bw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_global",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_fw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_bw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_offset_global",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_offset_fw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_raw_offset_bw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_global",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_fw",
    "output\registries\families\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_bw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_pointwise_control_global",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_pointwise_control_fw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_pointwise_control_bw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_global",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_fw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_centered_shape_bw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_offset_global",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_offset_fw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_raw_offset_bw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_global",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_fw",
    "output\training_runs\track2g_curve_aware_harmonic_residual_offset_full_curve_composite_bw",
...
function Save-ArtifactSyncManifest {
    ...
    if ($SyncFullTrack2CampaignResultPlots) {
        Add-ArtifactSyncRelativePath -RelativePath "doc\reports\campaign_results\track_2\verification_plots"
    }
    else {
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("track2g_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("track2g_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("track2g_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2g_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("track2g_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\track2g_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
...
if (`$null -eq `$manifestPath) {
        throw 'No remote Track 2G artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Track 2G artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Track 2G refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
    Invoke-RemoteSourceSync

    $remoteScriptPath = "scripts\campaigns\track_2\run_track2g_track2_verification_refresh.ps1"
    $remoteCommand = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
...
"@
    ...
    Write-StatusLine "DONE" "Remote Track 2G refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Track 2G verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            "--next-step",
            "Use the Track 2G curve-aware evidence when comparing later structured and dispersion-aware branches.",
            "--output-report-path",
            $officialReportPath,
            "--operator-log-root",
            $logRoot
...
Write-StatusLine "DONE" "Track 2G verification refresh completed"
