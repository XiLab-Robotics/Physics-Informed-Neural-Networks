...
EXPECTED_PROFILE_DICTIONARY = {
    "gru_offset_residual": "gru",
    "causal_tcn_offset_residual": "causal_tcn",
}
...
def resolve_surface_key(training_variant: str) -> str:
    ...
    """Resolve the canonical surface key from training variant metadata."""
...
def validate_queue_matrix(queue_config_list: list[dict[str, Any]]) -> None:
    ...
    for queue_config in queue_config_list:
        ...
        assert metadata.get("probe_group") == "latent_state_hysteresis"
        assert experiment.get("model_type") == "latent_state_hysteresis_probe"

        loss_profile = str(metadata.get("loss_profile", ""))
        assert loss_profile in EXPECTED_PROFILE_DICTIONARY, f"Unexpected 2H-L profile | {loss_profile}"
        ...
        assert float(model.get("offset_scale", 0.0)) > 0.0, "Offset scale must be positive"
        assert float(model.get("residual_scale", 0.0)) > 0.0, "Residual scale must be positive"
        forbidden_inputs = str(metadata.get("forbidden_inputs", ""))
        assert "measured TE" in forbidden_inputs and "future samples" in forbidden_inputs, (
            "Forbidden-input contract is missing from metadata"
        )
...
def validate_model_instantiation(queue_config_list: list[dict[str, Any]]) -> None:
    ...
    for queue_config in queue_config_list:
        ...
        assert getattr(created_model, "readout_position", None) == "last", (
            f"Created model readout is not causal-last | {queue_config['queue_path']}"
        )
...
def validate_one_batch_outputs(queue_config_list: list[dict[str, Any]]) -> None:
    ...
    for queue_config in queue_config_list:
        prepared_config = shared_training_infrastructure.prepare_output_artifact_training_config(
            ...
            run_name_suffix="track2h_latent_state_hysteresis_validation",
        )
        datamodule, regression_backbone, regression_module, _normalization_statistics = (
        ...
        latent_state_tensor = batch_output_dictionary.get("latent_state_tensor")
        assert deterministic_prediction_tensor.shape == target_tensor.shape, (
            f"Prediction shape mismatch | {queue_config['queue_path']}"
        )
        assert latent_state_tensor is not None, f"Missing latent-state diagnostics | {queue_config['queue_path']}"
        assert latent_state_tensor.shape[-1] == int(queue_config["model"]["latent_hidden_size"]), (
            f"Latent-state size mismatch | {queue_config['queue_path']}"
        )
        assert hasattr(regression_backbone, "compute_auxiliary_output_dictionary"), (
            f"Backbone does not expose auxiliary outputs | {queue_config['queue_path']}"
        )
