param(
    ...
    [string]$OutputSuffix = "polished_dataset_te_curve_verification_refresh_2026_07_02",
    [string]$ReportDate = "2026-07-02",
    [switch]$ResumeAfterVisualReports,
...
$inventoryBuilderPath = "scripts\paper_reimplementation\rcim_ml_compensation\reference_family_vs_feedforward\build_polished_rcim_reference_inventories.py"
...
$polishedPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\polished_model_development_registry"
...
function Save-ArtifactSyncManifest {
    ...
    Add-ArtifactSyncRelativePath -RelativePath "output\validation_checks\rcim_model_bank_reproduction\reference_inventories"
    if ($SyncFullTrack2CampaignResultPlots) {
        ...
        Add-ArtifactSyncRelativePath -RelativePath $polishedPlotDirectory
    }
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("polished_track2_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("polished_track2_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("polished_track2_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\polished_track2_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("polished_track2_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\polished_track2_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
...
if (`$null -eq `$manifestPath) {
    throw 'No remote polished TE Curve Verification Pipeline artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
    throw 'The remote polished artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching polished TE Curve Verification Pipeline refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
    Invoke-RemoteSourceSync

    $remoteScriptPath = "scripts\campaigns\track_2\run_polished_dataset_track2_verification_refresh.ps1"
$remoteCommand = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
& '.\$remoteScriptPath' -CondaEnvironmentName '$RemoteCondaEnvironmentName' -OutputSuffix '$OutputSuffix' -ReportDate '$ReportDate'$(if ($ResumeAfterVisualReports) { " -ResumeAfterVisualReports" } else { "" })$(if ($SkipVisualReports) { " -SkipVisualReports" } else { "" })$(if ($SkipPdfExport) { " -SkipPdfExport" } else { "" })$(if ($SyncFullTrack2CampaignResultPlots) { " -SyncFullTrack2CampaignResultPlots" } else { "" })
exit `$LASTEXITCODE
"@

    Invoke-RemotePowerShellText -RemoteScriptText $remoteCommand
    Invoke-RemoteArtifactManifestSync
    Write-StatusLine "DONE" "Remote polished TE Curve Verification Pipeline refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local polished TE Curve Verification Pipeline refresh"
...
if ($ResumeAfterVisualReports) {
    Write-StatusLine "INFO" "Resume mode: reusing the latest matrix, collage, and overlay artifacts; starting from visual source coverage"
}

if (-not $ResumeAfterVisualReports) {
    Invoke-LoggedCondaPython `
        -StepName "00_polished_rcim_reference_inventories" `
        -ArgumentList @(
            "-B",
            $inventoryBuilderPath
        )
    Add-ArtifactSyncRelativePath -RelativePath "output\validation_checks\rcim_model_bank_reproduction\reference_inventories"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            $ReportDate,
            "--refresh-label",
            "polished-dataset RCIM and full model-development refresh",
            "--candidate-source-label",
            "polished_model_development_registry",
            "--candidate-source-label",
            "polished_rcim_model_bank_reproduction",
            "--decision",
            "accepted official polished-dataset TE Curve Verification Pipeline refresh",
            "--next-step",
            "Use the accepted polished periodic GRU sequence family as the model-development baseline, keep the polished RCIM ET19 forward bank as the polished refreshed-source forward reference-bank leader, and decide next between deployment-facing export packaging and Wave 5.2 / Wave 6 integration planning.",
            "--output-report-path",
            $officialReportPath,
            "--operator-log-root",
            $logRoot
...
Write-StatusLine "DONE" "Polished TE Curve Verification Pipeline refresh completed"
