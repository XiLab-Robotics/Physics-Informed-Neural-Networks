...
CAMPAIGN_ROOT = Path(
    ...
    "2026-06-14_wave3_harmonic_prior_residual_campaign"
)
...
PLANNING_REPORT_PATH = Path(
    ...
    "2026-06-14-19-54-55_wave3_harmonic_prior_residual_campaign_plan_report.md"
)
TECHNICAL_DOCUMENT_PATH = Path(
    "doc/technical/2026-06/2026-06-14/"
    "2026-06-14-19-54-55_wave3_harmonic_prior_residual_campaign_package.md"
)
LAUNCHER_PATH = Path("scripts/campaigns/wave_3/run_wave3_harmonic_prior_residual_campaign.ps1")
VALIDATOR_PATH = Path("scripts/campaigns/wave_3/validate_wave3_harmonic_prior_residual_campaign.py")
LAUNCHER_NOTE_PATH = Path("doc/scripts/campaigns/wave_3/run_wave3_harmonic_prior_residual_campaign.md")
...
LOW_ORDER_HARMONIC_INDEX_LIST = [0, 1]
STABLE_MIDDLE_HARMONIC_INDEX_LIST = [3, 39, 40, 78, 81]
HIGH_ORDER_HARMONIC_INDEX_LIST = [156, 162, 240]
...
LOSS_PROFILE_DICTIONARY = {
    "pointwise_control": {
        ...
        "pointwise_loss": "mse",
        "intervention": "wave3_structured_control",
        "notes": "Deterministic MSE control for the explicit harmonic-prior residual architecture.",
    },
    "smooth_l1_structured": {
        "queue_label": "smooth_l1_structured",
...
def build_base_training_config(loss_profile_name: str) -> dict[str, Any]:
    ...
    loss_dictionary: dict[str, Any] = {
        ...
        "pointwise_loss": loss_profile["pointwise_loss"],
    ...
    if "huber_delta" in loss_profile:
        loss_dictionary["huber_delta"] = float(loss_profile["huber_delta"])
...
def build_queue_config(queue_index: int, surface_key: str, loss_profile_name: str) -> dict[str, Any]:
    ...
    dataset_file_name = str(direction_metadata["dataset_file_name"])
    loss_profile = LOSS_PROFILE_DICTIONARY[loss_profile_name]
    model_family = f"wave3_harmonic_prior_residual_{loss_profile_name}_{direction_token}"
    run_name = f"te_wave3_harmonic_prior_residual_{loss_profile_name}_{direction_token}"
    ...
    return {
        ...
        "metadata": {
            ...
            "phase_name": "wave3_harmonic_prior_residual",
            "campaign_config_id": model_family,
            "queue_index": queue_index,
            "intervention": loss_profile["intervention"],
            "probe_group": "wave3_harmonic_prior_residual",
...
def write_active_campaign_state(queue_path_list: list[Path], dataset_variant_path_list: list[Path], readme_path: Path) -> None:
    ...
    active_state = {
        "status": "prepared",
        "campaign_name": CAMPAIGN_NAME,
        "prepared_at": "2026-06-14T19:54:55+02:00",
