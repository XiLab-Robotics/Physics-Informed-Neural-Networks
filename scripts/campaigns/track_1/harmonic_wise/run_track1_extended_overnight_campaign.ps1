...
$planningReportPath = "doc\reports\campaign_plans\track_1\harmonic_wise\2026-04-13-13-27-37_track1_extended_overnight_campaign_plan_report.md"
$campaignName = "track1_extended_overnight_campaign_2026_04_13_13_31_57"
$campaignOutputRoot = Join-Path "output\training_campaigns\track1\harmonic_wise" $campaignName
...
for ($runIndex = 0; $runIndex -lt $campaignRunList.Count; $runIndex++) {
    ...
    if ($nativeExitCode -ne 0) {
        Write-Host "[ERROR] RCIM Model-Bank Reproduction extended overnight campaign run failed | $configPath" -ForegroundColor Red
...
Write-Host ""
Write-Host "[DONE] RCIM Model-Bank Reproduction extended overnight campaign completed successfully" -ForegroundColor Green
