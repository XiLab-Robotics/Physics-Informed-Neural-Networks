...
$planningReportPath = "doc\reports\campaign_plans\track_1\svm\2026-04-14-21-42-47_track1_svm_micro_closure_campaign_plan_report.md"
...
$campaignConfigPathList = @(
    (Join-Path $campaignConfigRoot "01_track1_svm_amplitude_micro_closure_baseline.yaml")
    (Join-Path $campaignConfigRoot "02_track1_svm_amplitude_micro_closure_seed23.yaml")
    (Join-Path $campaignConfigRoot "03_track1_svm_amplitude_micro_closure_split15.yaml")
    (Join-Path $campaignConfigRoot "04_track1_svm_amplitude_40_only.yaml")
    (Join-Path $campaignConfigRoot "05_track1_svm_amplitude_240_only.yaml")
    (Join-Path $campaignConfigRoot "06_track1_svm_phase_micro_closure_baseline.yaml")
    (Join-Path $campaignConfigRoot "07_track1_svm_phase_micro_closure_split15.yaml")
    (Join-Path $campaignConfigRoot "08_track1_svm_phase_micro_closure_seed23.yaml")
)
...
for ($configIndex = 0; $configIndex -lt $campaignConfigPathList.Count; $configIndex++) {
    ...
    if ($nativeExitCode -ne 0) {
        Write-Host "[ERROR] Exact-paper SVM micro-closure campaign run failed | $configPath" -ForegroundColor Red
...
Write-Host ""
Write-Host "[DONE] Exact-paper SVM micro-closure campaign completed successfully" -ForegroundColor Green
