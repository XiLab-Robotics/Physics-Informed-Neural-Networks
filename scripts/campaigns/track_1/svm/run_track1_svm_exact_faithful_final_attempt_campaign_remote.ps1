...
$planningReportPath = "doc\reports\campaign_plans\track_1\svm\2026-04-17-11-44-20_track1_svm_exact_faithful_final_attempt_campaign_plan_report.md"
...
$campaignConfigPathList = @(
    (Join-Path $campaignConfigRoot "01_track1_svr_exact_faithful_amplitude_pair_repeat.yaml")
    (Join-Path $campaignConfigRoot "02_track1_svr_exact_faithful_amplitude_40_repeat.yaml")
    (Join-Path $campaignConfigRoot "03_track1_svr_exact_faithful_amplitude_240_repeat.yaml")
    (Join-Path $campaignConfigRoot "04_track1_svr_exact_faithful_phase_162_repeat.yaml")
)
$runNameList = @(
    "track1_svr_exact_faithful_amplitude_pair_repeat"
    "track1_svr_exact_faithful_amplitude_40_repeat"
    "track1_svr_exact_faithful_amplitude_240_repeat"
...
$initialRemoteRunLogPath = Join-Path $campaignLogRoot "01_track1_svr_exact_faithful_amplitude_pair_repeat.log"
...
$requiredRemoteSourcePathList = @($resolvedCampaignConfigPathList + @($resolvedPlanningReportPath, "scripts\campaigns\track_1\svm\run_track1_svm_exact_faithful_final_attempt_campaign.ps1", "scripts\campaigns\infrastructure\shared_streaming_campaign_launcher.ps1"))
...
`$launcherPath = Join-Path '$remoteExecutionRoot' 'scripts\campaigns\track_1\svm\run_track1_svm_exact_faithful_final_attempt_campaign.ps1'
...
Emit-RemoteStatusLine ('[DONE] Exact-paper SVM faithful-final campaign completed successfully')
...
foreach (`$runName in `$runNameList) {
    ...
    `$reportFile = Get-ChildItem -LiteralPath 'doc\reports\analysis\validation_checks' -Recurse -File |
...
$remoteRunResult = Invoke-RemotePowerShellScriptWithStreamingLog `
    ...
    -RemoteKillMatchPatternList @(
        ...
        "2026-04-17_track1_svm_exact_faithful_final_attempt_campaign"
    ) `
