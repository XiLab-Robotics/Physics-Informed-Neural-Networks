...
for ($configIndex = 0; $configIndex -lt $campaignConfigPathList.Count; $configIndex++) {
    ...
    if ($nativeExitCode -ne 0) {
        Write-Host "[ERROR] Exact-paper SVR reference-grid campaign run failed | $configPath" -ForegroundColor Red
...
Write-Host ""
Write-Host "[DONE] Exact-paper SVR reference-grid repair campaign completed successfully" -ForegroundColor Green
