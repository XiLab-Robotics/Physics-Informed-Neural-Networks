"""Prepare the forward-only remote diagnostic campaign for RCIM Model-Bank Reproduction."""
...
PLANNING_REPORT_RELATIVE_PATH = (
    ...
    "2026-04-25-16-10-04_track1_forward_original_dataset_remote_diagnostic_campaign_plan_report.md"
)
LAUNCHER_RELATIVE_PATH = (
    "scripts/campaigns/track_1/exact_paper/"
    "run_track1_forward_original_dataset_remote_diagnostic_campaign.ps1"
)
...
LAUNCHER_NOTE_RELATIVE_PATH = (
    "doc/scripts/campaigns/run_track1_forward_original_dataset_remote_diagnostic_campaign.md"
)
CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE = "output/training_campaigns/track1/exact_paper/forward/{campaign_name}"
VALIDATION_OUTPUT_ROOT = "output/validation_checks/paper_reimplementation_rcim_original_dataset_exact_model_bank_remote_diagnostic"
...
def build_campaign_config(family_name: str) -> dict:
    ...
    """Build one family-specific diagnostic config payload."""
    ...
    baseline_payload["experiment"]["run_name"] = (
        f"track1_original_dataset_forward_{family_slug}_remote_diagnostic"
    )
    baseline_payload["experiment"]["model_family"] = (
        "paper_reimplementation_rcim_original_dataset_exact_model_bank_remote_diagnostic"
    )
    baseline_payload["experiment"]["model_type"] = f"exact_model_bank_forward_{family_slug}_remote_diagnostic"
...
def build_campaign_readme_markdown(campaign_slug: str, family_name: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "- purpose: `remote ONNX and warning diagnostic surface`",
            "",
        ]
    )
...
def main() -> None:
    ...
    """Prepare the complete diagnostic campaign package and active state."""
    ...
    campaign_name = f"track1_forward_original_dataset_remote_diagnostic_campaign_{timestamp_string}"
    ...
    for family_name in exact_paper_model_bank_support.EXACT_FAMILY_ORDER:
        campaign_slug = (
            ...
            f"{family_name.lower()}_remote_diagnostic"
        )
    ...
    active_campaign_payload = {
        ...
        "campaign_output_directory": CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE.format(campaign_name=campaign_name).replace("/", "\\"),
        ...
        "interruption_note": (
            "Supersedes the interrupted 400-run bidirectional mega campaign after the "
            "remote ONNX export dependency failure and MLP convergence-warning investigation."
        ),
    }
    save_yaml_file(active_campaign_payload, ACTIVE_CAMPAIGN_STATE_PATH)
    print(f"[DONE] Prepared remote diagnostic queue count | {len(queue_config_relative_path_list)}", flush=True)
