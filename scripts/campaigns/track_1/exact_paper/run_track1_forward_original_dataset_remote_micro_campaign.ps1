...
function Get-CampaignQueueBundle {
    ...
    $TemporaryPythonScriptPath = Join-Path $env:TEMP ("track1_forward_remote_micro_queue_bundle_{0}.py" -f ([guid]::NewGuid().ToString("N")))
...
if ($Remote) {
    & ".\scripts\campaigns\track_1\exact_paper\run_exact_paper_campaign_remote.ps1" `
        ...
        -LauncherRelativePath "scripts\campaigns\track_1\exact_paper\run_track1_forward_original_dataset_remote_micro_campaign.ps1" `
        ...
        -ValidationOutputRoot "output\validation_checks\paper_reimplementation_rcim_original_dataset_exact_model_bank_remote_micro" `
...
$CampaignConfigRoot = Join-Path $ProjectRoot "config\paper_reimplementation\rcim_ml_compensation\original_dataset_exact_model_bank\campaigns\track1\exact_paper\forward_remote_micro"
$RelativeConfigPrefix = "config\paper_reimplementation\rcim_ml_compensation\original_dataset_exact_model_bank\campaigns\track1\exact_paper\forward_remote_micro\"
