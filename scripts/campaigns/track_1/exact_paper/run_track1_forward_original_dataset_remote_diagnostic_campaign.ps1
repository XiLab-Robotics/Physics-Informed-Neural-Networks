...
function Get-CampaignQueueBundle {
    ...
    $TemporaryPythonScriptPath = Join-Path $env:TEMP ("track1_forward_remote_diagnostic_queue_bundle_{0}.py" -f ([guid]::NewGuid().ToString("N")))
...
if ($Remote) {
    & ".\scripts\campaigns\track_1\exact_paper\run_exact_paper_campaign_remote.ps1" `
        ...
        -LauncherRelativePath "scripts\campaigns\track_1\exact_paper\run_track1_forward_original_dataset_remote_diagnostic_campaign.ps1" `
        ...
        -ValidationOutputRoot "output\validation_checks\paper_reimplementation_rcim_original_dataset_exact_model_bank_remote_diagnostic" `
...
foreach ($ConfigRelativePath in @($CampaignQueueBundle.queue_config_path_list)) {
    ...
    Write-Host "[INFO] Running remote diagnostic validation | $ConfigPath" -ForegroundColor Cyan
    conda run -n $CondaEnvironmentName $PythonExecutable $RunnerPath --config-path $ConfigPath --output-suffix remote_diagnostic
    if ($LASTEXITCODE -ne 0) {
        throw "Remote diagnostic run failed | $ConfigPath"
    }
}
