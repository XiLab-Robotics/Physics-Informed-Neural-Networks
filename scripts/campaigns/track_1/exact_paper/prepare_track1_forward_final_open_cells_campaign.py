"""Prepare the final forward-only open-cells campaign package for RCIM Model-Bank Reproduction."""
...
PLANNING_REPORT_RELATIVE_PATH = (
    ...
    "2026-04-28-00-16-01_track1_forward_final_open_cells_campaign_plan_report.md"
)
LAUNCHER_RELATIVE_PATH = (
    "scripts/campaigns/track_1/exact_paper/"
    "run_track1_forward_final_open_cells_campaign.ps1"
)
...
LAUNCHER_NOTE_RELATIVE_PATH = (
    "doc/scripts/campaigns/run_track1_forward_final_open_cells_campaign.md"
)
CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE = (
    "output/training_campaigns/track1/exact_paper/forward_final_open_cells/{campaign_name}"
)
...
RED_ESCALATION_ATTEMPT_SPECIFICATION_LIST = [
    AttemptSpecification(0.25, 0.10, 41, "red_escalation"),
    AttemptSpecification(0.25, 0.10, 43, "red_escalation"),
    AttemptSpecification(0.20, 0.15, 47, "red_escalation"),
    AttemptSpecification(0.20, 0.15, 53, "red_escalation"),
]
FORWARD_RESIDUAL_REPAIR_ITEM_LIST = [
    ForwardResidualRepairItem("ERT", "ampl", 156, "yellow_only", ("Table 2",)),
    ForwardResidualRepairItem("ERT", "ampl", 162, "red", ("Table 2",)),
    ForwardResidualRepairItem("ERT", "ampl", 240, "yellow_multi_surface", ("Table 2", "Table 3")),
    ForwardResidualRepairItem("GBM", "ampl", 162, "red", ("Table 2", "Table 3")),
    ...
    ForwardResidualRepairItem("XGBM", "ampl", 240, "yellow_only", ("Table 3",)),
    ForwardResidualRepairItem("LGBM", "phase", 81, "yellow_only", ("Table 4",)),
]
...
def build_campaign_config(
    ...
    baseline_payload["experiment"]["run_name"] = (
        f"track1_forward_{family_slug}_{target_slug}_final_open_cells_{attempt_slug}"
    )
    baseline_payload["experiment"]["model_family"] = (
        "paper_reimplementation_rcim_original_dataset_exact_model_bank_forward_final_open_cells"
    )
    baseline_payload["experiment"]["model_type"] = (
        f"exact_model_bank_forward_final_open_cells_{family_slug}_{target_slug}"
    )
...
def build_campaign_directory_slug(repair_item: ForwardResidualRepairItem) -> str:
    ...
    return (
        ...
        f"h{repair_item.harmonic_order}_final_open_cells"
    )
...
def build_campaign_config_path(
    ...
    filename = (
        ...
        f"{repair_item.target_kind}_h{repair_item.harmonic_order}_final_open_cells_"
...
def main() -> None:
    ...
    """Prepare the complete final forward open-cells campaign package."""
    ...
    campaign_name = f"track1_forward_final_open_cells_campaign_{timestamp_string}"
    ...
    active_campaign_payload = {
        ...
        "interruption_note": (
            "Prepared as the final forward-only residual repair wave after the "
            "completed forward open-cell repair closeout."
        ),
    }
    save_yaml_file(active_campaign_payload, ACTIVE_CAMPAIGN_STATE_PATH)
    print(
        f"[DONE] Prepared final forward residual queue count | "
