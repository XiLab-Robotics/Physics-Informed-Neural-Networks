"""Prepare the final targeted forward-only last-three-open-cells campaign package for RCIM Model-Bank Reproduction."""
...
PLANNING_REPORT_RELATIVE_PATH = (
    ...
    "2026-04-29-14-28-06_track1_forward_last_three_open_cells_campaign_plan_report.md"
)
LAUNCHER_RELATIVE_PATH = (
    "scripts/campaigns/track_1/exact_paper/"
    "run_track1_forward_last_three_open_cells_campaign.ps1"
)
...
LAUNCHER_NOTE_RELATIVE_PATH = (
    "doc/scripts/campaigns/run_track1_forward_last_three_open_cells_campaign.md"
)
CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE = (
    "output/training_campaigns/track1/exact_paper/forward_last_three_open_cells/{campaign_name}"
)
...
STUBBORN_SINGLE_EXTENSION_ATTEMPT_SPECIFICATION_LIST = [
    AttemptSpecification(0.14, 0.16, 109, "stubborn_single_extension"),
    AttemptSpecification(0.14, 0.16, 113, "stubborn_single_extension"),
    AttemptSpecification(0.26, 0.08, 127, "stubborn_single_extension"),
    AttemptSpecification(0.26, 0.08, 131, "stubborn_single_extension"),
    AttemptSpecification(0.08, 0.22, 137, "stubborn_single_extension"),
    AttemptSpecification(0.08, 0.22, 139, "stubborn_single_extension"),
    AttemptSpecification(0.18, 0.18, 149, "stubborn_single_extension"),
    AttemptSpecification(0.18, 0.18, 151, "stubborn_single_extension"),
    AttemptSpecification(0.28, 0.06, 157, "stubborn_single_extension"),
    AttemptSpecification(0.28, 0.06, 163, "stubborn_single_extension"),
    AttemptSpecification(0.06, 0.24, 167, "stubborn_single_extension"),
    AttemptSpecification(0.06, 0.24, 173, "stubborn_single_extension"),
]
FORWARD_RESIDUAL_REPAIR_ITEM_LIST = [
    ForwardResidualRepairItem("ERT", "ampl", 156, "yellow_single_surface", ("Table 2",)),
    ForwardResidualRepairItem("ERT", "ampl", 240, "yellow_single_surface_stubborn", ("Table 2",)),
    ForwardResidualRepairItem("GBM", "ampl", 162, "yellow_single_surface", ("Table 2",)),
]
...
def build_attempt_specification_list(
    ...
    if repair_item.severity_label == "yellow_single_surface_stubborn":
        attempt_specification_list.extend(STUBBORN_SINGLE_EXTENSION_ATTEMPT_SPECIFICATION_LIST)
...
def build_campaign_config(
    ...
    baseline_payload["experiment"]["run_name"] = (
        f"track1_forward_{family_slug}_{target_slug}_last_three_open_cells_{attempt_slug}"
    )
    baseline_payload["experiment"]["model_family"] = (
        "paper_reimplementation_rcim_original_dataset_exact_model_bank_forward_last_three_open_cells"
    )
    baseline_payload["experiment"]["model_type"] = (
        f"exact_model_bank_forward_last_three_open_cells_{family_slug}_{target_slug}"
    )
...
def build_campaign_directory_slug(repair_item: ForwardResidualRepairItem) -> str:
    ...
    return (
        ...
        f"h{repair_item.harmonic_order}_last_three_open_cells"
    )
...
def build_campaign_config_path(
    ...
    filename = (
        ...
        f"{repair_item.target_kind}_h{repair_item.harmonic_order}_last_three_open_cells_"
...
def main() -> None:
    ...
    campaign_name = f"track1_forward_last_three_open_cells_campaign_{timestamp_string}"
    ...
    active_campaign_payload = {
        ...
        "launch_command_list": [
            ".\\scripts\\campaigns\\track1\\exact_paper\\run_track1_forward_last_three_open_cells_campaign.ps1 -Remote",
        ],
        ...
        "interruption_note": (
            "Prepared as the next forward-only residual repair wave "
            "after the completed forward last-four-open-cells closeout."
        ),
    }
    save_yaml_file(active_campaign_payload, ACTIVE_CAMPAIGN_STATE_PATH)
    print(
        f"[DONE] Prepared final targeted forward residual queue count | "
