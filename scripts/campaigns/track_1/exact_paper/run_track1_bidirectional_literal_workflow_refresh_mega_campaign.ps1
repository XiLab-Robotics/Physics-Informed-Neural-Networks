...
function Get-CampaignQueueBundle {
    ...
    $TemporaryPythonScriptPath = Join-Path $env:TEMP ("track1_bidirectional_literal_refresh_queue_bundle_{0}.py" -f ([guid]::NewGuid().ToString("N")))
...
if ($Remote) {
    & ".\scripts\campaigns\track_1\exact_paper\run_exact_paper_campaign_remote.ps1" `
        ...
        -LauncherRelativePath "scripts\campaigns\track_1\exact_paper\run_track1_bidirectional_literal_workflow_refresh_mega_campaign.ps1" `
...
for ($ConfigIndex = 0; $ConfigIndex -lt $QueueConfigCount; $ConfigIndex++) {
    ...
    Write-Host ("[INFO] Running literal-workflow refresh validation | {0}" -f $ConfigPath) -ForegroundColor Cyan
    ...
    if ($NativeExitCode -ne 0) {
        throw "Literal-workflow refresh mega campaign run failed | $ConfigPath"
    }
...
Write-Host "[DONE] RCIM Model-Bank Reproduction bidirectional literal-workflow refresh mega campaign completed" -ForegroundColor Green
