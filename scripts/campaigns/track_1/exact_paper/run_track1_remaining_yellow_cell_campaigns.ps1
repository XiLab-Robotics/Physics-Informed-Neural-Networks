...
$launcherRelativePathList = @(
    ...
    "scripts\campaigns\track_1\exact_paper\run_track1_mlp_remaining_yellow_cell_campaign.ps1"
    "scripts\campaigns\track_1\exact_paper\run_track1_et_remaining_yellow_cell_campaign.ps1"
    "scripts\campaigns\track_1\exact_paper\run_track1_ert_remaining_yellow_cell_campaign.ps1"
    "scripts\campaigns\track_1\exact_paper\run_track1_hgbm_remaining_yellow_cell_campaign.ps1"
    "scripts\campaigns\track_1\exact_paper\run_track1_xgbm_remaining_yellow_cell_campaign.ps1"
)

Write-Host "[INFO] Aggregate RCIM Model-Bank Reproduction Remaining Yellow-Cell Launcher Count | $($launcherRelativePathList.Count)" -ForegroundColor Cyan
...
for ($launcherIndex = 0; $launcherIndex -lt $launcherRelativePathList.Count; $launcherIndex++) {
    ...
    if ($nativeExitCode -ne 0) {
        Write-Host "[ERROR] Aggregate remaining-yellow-cell launcher failed | $launcherRelativePath" -ForegroundColor Red
        exit $nativeExitCode
    }
}

Write-Host ""
Write-Host "[DONE] RCIM Model-Bank Reproduction remaining yellow-cell campaign sequence completed successfully" -ForegroundColor Green
exit 0
