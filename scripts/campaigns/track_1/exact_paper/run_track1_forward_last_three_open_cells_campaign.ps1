...
function Get-CampaignQueueBundle {
    ...
    $TemporaryPythonScriptPath = Join-Path $env:TEMP ("track1_forward_last_three_open_cells_queue_bundle_{0}.py" -f ([guid]::NewGuid().ToString("N")))
...
if ($Remote) {
    & ".\scripts\campaigns\track_1\exact_paper\run_exact_paper_campaign_remote.ps1" `
        ...
        -LauncherRelativePath "scripts\campaigns\track_1\exact_paper\run_track1_forward_last_three_open_cells_campaign.ps1" `
        ...
        -ValidationOutputRoot "output\validation_checks\paper_reimplementation_rcim_original_dataset_exact_model_bank_forward_last_three_open_cells" `
...
$CampaignConfigRoot = Join-Path $ProjectRoot "config\paper_reimplementation\rcim_ml_compensation\original_dataset_exact_model_bank\campaigns\track1\exact_paper\forward_last_three_open_cells"
$RelativeConfigPrefix = "config\paper_reimplementation\rcim_ml_compensation\original_dataset_exact_model_bank\campaigns\track1\exact_paper\forward_last_three_open_cells\"
