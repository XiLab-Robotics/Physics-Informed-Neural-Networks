...
$CampaignName = "wave52r_stage9_temporal_analytical_residual_models_2026_07_29"
$CampaignScriptPath = "scripts/campaigns/wave_5_2/run_wave52r_stage9_temporal_analytical_residual_models.py"
$PlanningReportPath = "doc/reports/campaign_plans/model_development_waves/wave_5_2/temporal_analytical_residual_models/2026-07-29-18-35-28_wave52r_stage9_temporal_analytical_residual_models_campaign_plan_report.md"
$TechnicalDocumentPath = "doc/technical/2026-07/2026-07-29/2026-07-29-18-35-28_wave52r_stage9_temporal_analytical_residual_models.md"
$ModelReportPath = "doc/reports/analysis/model_development_waves/wave_5_2/physics_guided_pinn_reassessment/[2026-07-29]/stage9_temporal_analytical_residual_models/stage9_temporal_analytical_residual_model_report.md"
$H04RunPath = "output/training_runs/complex_harmonic_coefficient_residuals/2026-07-28-16-17-13__stage5_h04"
$AcceptedGruRunPath = "output/training_runs/periodic_gru_sequence/2026-06-26-15-05-38__te_periodic_gru_sequence_fw"
$script:LastPythonExitCode = 0

function Write-Stage9Status {
...
foreach ($RequiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $RequiredPath)) {
        throw "Required Stage 9 path is missing | $RequiredPath"
    }
}

Write-Stage9Status -Label "INFO" -Message "Campaign: $CampaignName"
Write-Stage9Status -Label "INFO" -Message "Scope: polished_dataset / setpoints / Fw"
Write-Stage9Status -Label "STEP" -Message "Running causal-state, anchor, split, replay, and leakage preflight."
Invoke-Stage9Python -ArgumentList @("-B", $CampaignScriptPath, "--preflight-only")
if ($script:LastPythonExitCode -ne 0) { exit $script:LastPythonExitCode }

if ($PreflightOnly -or -not $Run) {
    Write-Stage9Status -Label "DONE" -Message "Preflight completed without training."
    exit 0
}

if (-not $Remote) {
    Write-Stage9Status -Label "STEP" -Message "Launching local Stage 9 campaign."
    Invoke-Stage9Python -ArgumentList @("-B", $CampaignScriptPath, "--run")
...
$RemoteWorkRoot = Join-Path $ProjectRoot ".temp\stage9_remote_campaign"
New-Item -ItemType Directory -Force -Path $RemoteWorkRoot | Out-Null
$LocalSourceArchivePath = Join-Path $RemoteWorkRoot "stage9_source.tar"
$LocalResultArchivePath = Join-Path $RemoteWorkRoot "stage9_results.tar"
$LocalRemoteScriptPath = Join-Path $RemoteWorkRoot "stage9_remote_run.ps1"
$SourcePathList = @(
    "scripts",
    "config",
    "doc",
...
`$RemoteSourceArchivePath = Join-Path `$env:USERPROFILE "stage9_source.tar"
`$RemoteResultArchivePath = Join-Path `$env:USERPROFILE "stage9_results.tar"
...
if (`$null -eq `$CampaignOutputDirectory) { throw "Stage 9 remote output was not found." }
...
& scp -q $LocalSourceArchivePath "${RemoteHostAlias}:stage9_source.tar"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& scp -q $LocalRemoteScriptPath "${RemoteHostAlias}:stage9_remote_run.ps1"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& ssh $RemoteHostAlias "powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File stage9_remote_run.ps1"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& scp -q "${RemoteHostAlias}:stage9_results.tar" $LocalResultArchivePath
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
