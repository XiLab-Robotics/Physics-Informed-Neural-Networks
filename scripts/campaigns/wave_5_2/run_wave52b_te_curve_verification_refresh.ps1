param(
    ...
    [string]$OutputSuffix = "wave52b_offset_harmonic_guided_track2_refresh_2026_07_02",
...
$newCandidateCampaignResultPlotDirectory = "doc\reports\campaign_results\track_2\verification_plots\wave52b_offset_harmonic_guided_registry"
...
$sourceSyncPathList = @(
    ...
    "output\registries\families\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_global",
    "output\registries\families\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_fw",
    "output\registries\families\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_bw",
    "output\training_runs\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_global",
    "output\training_runs\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_fw",
    "output\training_runs\wave52b_offset_harmonic_guided_offset_centered_shape_harmonic_bw",
...
function Invoke-RemotePowerShellText {
    ...
    $localTemporaryScriptPath = Join-Path $temporaryRoot ("wave52b_remote_{0}.ps1" -f $scriptIdentifier)
    $remoteTemporaryDirectoryPath = "C:\Temp\standardml_track2_remote"
    $remoteTemporaryScriptPath = Join-Path $remoteTemporaryDirectoryPath ("wave52b_remote_{0}.ps1" -f $scriptIdentifier)
...
function Invoke-RemoteSourceSync {
    ...
    $localArchivePath = Join-Path $syncRoot ("wave52b_source_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\wave52b_source_sync.tar"
...
function Invoke-RemoteArtifactManifestSync {
    $syncRoot = Join-Path $projectRoot "output\validation_checks\track2_operator_launch_logs\remote_sync"
    New-Item -ItemType Directory -Force -Path $syncRoot | Out-Null
    $localArchivePath = Join-Path $syncRoot ("wave52b_artifact_sync_{0}.tar" -f (Get-Date -Format "yyyy-MM-dd-HH-mm-ss"))
    $remoteArchivePath = Join-Path $RemoteRepositoryPath ".temp\wave52b_artifact_sync.tar"
    $remoteArchiveScpPath = Convert-ToScpRemotePath -WindowsPath $remoteArchivePath
...
if (`$null -eq `$manifestPath) {
        throw 'No remote Wave 5.2B artifact sync manifest found.'
}
`$existingPathList = @()
...
if (`$existingPathList.Count -eq 0) {
        throw 'The remote Wave 5.2B artifact sync manifest did not contain existing paths.'
}
...
if ($Remote) {
    ...
    Write-StatusLine "INFO" ("Launching Wave 5.2B refresh remotely | host={0} | repo={1}" -f $RemoteHostAlias, $RemoteRepositoryPath)
    Invoke-RemoteSourceSync

    $remoteScriptPath = "scripts\campaigns\wave_5_2\run_wave52b_te_curve_verification_refresh.ps1"
    $remoteCommand = @"
Set-Location -LiteralPath '$RemoteRepositoryPath'
...
"@
    ...
    Write-StatusLine "DONE" "Remote Wave 5.2B refresh completed and artifacts synchronized locally"
    exit 0
}

Write-StatusLine "INFO" "Preparing local Wave 5.2B verification refresh"
...
if (-not $SkipVisualReports) {
    ...
    Invoke-LoggedCondaPython `
        ...
        -ArgumentList @(
            ...
            "wave52b_offset_harmonic_guided_registry",
            "--decision",
            "pending operator review; do not promote from scalar metrics alone",
            "--next-step",
...
Write-StatusLine "DONE" "Wave 5.2B verification refresh completed"
