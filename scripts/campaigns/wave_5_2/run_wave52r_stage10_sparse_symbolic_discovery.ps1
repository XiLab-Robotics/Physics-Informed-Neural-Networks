...
$CampaignName = "wave52r_stage10_sparse_symbolic_discovery_2026_07_29"
$CampaignScriptPath = "scripts/campaigns/wave_5_2/run_wave52r_stage10_sparse_symbolic_discovery.py"
$PlanningReportPath = "doc/reports/campaign_plans/model_development_waves/wave_5_2/sparse_symbolic_formulation_discovery/2026-07-29-20-07-54_wave52r_stage10_sparse_and_symbolic_formulation_discovery_campaign_plan_report.md"
$TechnicalDocumentPath = "doc/technical/2026-07/2026-07-29/2026-07-29-20-07-54_wave52r_stage10_sparse_and_symbolic_formulation_discovery.md"
$ModelReportPath = "doc/reports/analysis/model_development_waves/wave_5_2/physics_guided_pinn_reassessment/[2026-07-29]/stage10_sparse_symbolic_formulation_discovery/stage10_sparse_symbolic_formulation_model_report.md"
...
foreach ($RequiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $RequiredPath)) {
        throw "Required Stage 10 path is missing | $RequiredPath"
    }
}

Write-Stage10Status -Label "INFO" -Message "Campaign: $CampaignName"
Write-Stage10Status -Label "INFO" -Message "Scope: polished_dataset / setpoints / Fw"
Write-Stage10Status -Label "STEP" -Message "Running split, library, reconstruction, leakage, and synthetic-recovery preflight."
Invoke-Stage10Python -ArgumentList @("-B", $CampaignScriptPath, "--preflight-only")
if ($script:LastPythonExitCode -ne 0) { exit $script:LastPythonExitCode }

if ($PreflightOnly -or -not $Run) {
    Write-Stage10Status -Label "DONE" -Message "Preflight completed without fitting the campaign."
    exit 0
}

if (-not $Remote) {
    Write-Stage10Status -Label "STEP" -Message "Launching local Stage 10 campaign."
    Invoke-Stage10Python -ArgumentList @("-B", $CampaignScriptPath, "--run")
...
$RemoteWorkRoot = Join-Path $ProjectRoot ".temp\stage10_remote_campaign"
New-Item -ItemType Directory -Force -Path $RemoteWorkRoot | Out-Null
$LocalSourceArchivePath = Join-Path $RemoteWorkRoot "stage10_source.tar"
$LocalResultArchivePath = Join-Path $RemoteWorkRoot "stage10_results.tar"
$LocalRemoteScriptPath = Join-Path $RemoteWorkRoot "stage10_remote_run.ps1"
$SourcePathList = @(
    "scripts",
    "config",
    "doc",
...
`$RemoteSourceArchivePath = Join-Path `$env:USERPROFILE "stage10_source.tar"
`$RemoteResultArchivePath = Join-Path `$env:USERPROFILE "stage10_results.tar"
...
if (`$null -eq `$CampaignOutputDirectory) { throw "Stage 10 remote output was not found." }
...
& scp -q $LocalSourceArchivePath "${RemoteHostAlias}:stage10_source.tar"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& scp -q $LocalRemoteScriptPath "${RemoteHostAlias}:stage10_remote_run.ps1"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& ssh $RemoteHostAlias "powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File stage10_remote_run.ps1"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& scp -q "${RemoteHostAlias}:stage10_results.tar" $LocalResultArchivePath
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
