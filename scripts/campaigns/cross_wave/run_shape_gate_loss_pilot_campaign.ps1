...
$CampaignName = "shape_gate_loss_pilot_2026_07_20"
$CampaignManifestPath = "config/training/shape_gate_loss_pilot/campaigns/2026-07-20_shape_gate_loss_pilot/campaign.yaml"
...
$PlanningReportPath = "doc/reports/campaign_plans/cross_wave/shape_gate_loss/2026-07-20-19-10-23_shape_gate_loss_pilot_campaign_plan_report.md"
$QueueRoot = "config\training\queue\shape_gate_loss_pilot"
$script:LastPythonExitCode = 0

function Write-ShapeGatePilotStatus {
    param(
        [string]$Label,
        [string]$Message
    )
...
function Invoke-ShapeGatePilotPython {
...
Write-ShapeGatePilotStatus -Label "INFO" -Message "Campaign: $CampaignName"
Write-ShapeGatePilotStatus -Label "INFO" -Message "Primary scope: polished_dataset setpoints Fw"
Write-ShapeGatePilotStatus -Label "INFO" -Message "Manifest: $CampaignManifestPath"
...
if ($RunOneBatchValidation) {
    Write-ShapeGatePilotStatus -Label "STEP" -Message "Running one-batch validation without launching training."
    Invoke-ShapeGatePilotPython -ArgumentList @(
        "-B",
...
if ($PreflightOnly) {
    Write-ShapeGatePilotStatus -Label "DONE" -Message "Preflight completed without training."
    exit 0
}
...
if ($EnqueueOnly) {
    ...
    Write-ShapeGatePilotStatus -Label "STEP" -Message "Enqueue-only verification enabled; training will not start."
}

Write-ShapeGatePilotStatus -Label "STEP" -Message "Launching local shape-gate loss pilot."
Invoke-ShapeGatePilotPython -ArgumentList $TrainingArgumentList
