...
CAMPAIGN_CONFIG_ROOT = (
    ...
    / "track1"
    / "exact_paper"
    / "bidirectional_literal_workflow_refresh"
)
...
PLANNING_REPORT_RELATIVE_PATH = (
    ...
    "2026-04-30-02-06-47_track1_bidirectional_literal_workflow_refresh_mega_campaign_plan_report.md"
)
LAUNCHER_RELATIVE_PATH = (
    "scripts/campaigns/track1/exact_paper/"
    "run_track1_bidirectional_literal_workflow_refresh_mega_campaign.ps1"
)
...
LAUNCHER_NOTE_RELATIVE_PATH = (
    ...
    "run_track1_bidirectional_literal_workflow_refresh_mega_campaign.md"
)
CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE = (
    "output/training_campaigns/track1/exact_paper/"
    "bidirectional_literal_workflow_refresh/{campaign_name}"
)
...
def build_campaign_config(
    ...
    baseline_payload["experiment"]["run_name"] = (
        f"track1_literal_workflow_refresh_{direction_label}_{family_slug}_{attempt_slug}"
    )
    ...
    baseline_payload["experiment"]["model_type"] = (
        f"exact_model_bank_{direction_label}_{family_slug}_literal_workflow_refresh_campaign"
    )
...
def build_campaign_config_path(
    ...
    filename = (
        f"{attempt_index:03d}_track1_literal_workflow_refresh_{direction_label}_{family_slug}_"
...
def main() -> None:
    ...
    campaign_name = (
        "track1_bidirectional_literal_workflow_refresh_mega_campaign_"
        f"{timestamp_string}"
    )
    ...
    for direction_label in ["forward", "backward"]:
        for family_name in exact_paper_model_bank_support.EXACT_FAMILY_ORDER:
            campaign_slug = (
                ...
                f"{family_name.lower()}_literal_workflow_refresh_mega_campaign"
            )
            ...
            for attempt_index, split_seed in enumerate(ATTEMPT_SEED_LIST, start=1):
                ...
                save_yaml_file(
                    build_campaign_config(direction_label, family_name, attempt_index, split_seed),
                    config_path,
                )
    ...
    active_campaign_payload = {
        ...
        "launch_command_list": [
            ".\\scripts\\campaigns\\track1\\exact_paper\\run_track1_bidirectional_literal_workflow_refresh_mega_campaign.ps1 -Remote",
        ],
    ...
    print(f"[DONE] Prepared literal-workflow refresh mega-campaign queue count | {len(queue_config_relative_path_list)}", flush=True)
