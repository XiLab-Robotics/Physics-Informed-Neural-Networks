...
PLANNING_REPORT_RELATIVE_PATH = (
    ...
    "2026-04-29-11-56-05_track1_forward_last_four_open_cells_campaign_plan_report.md"
)
LAUNCHER_RELATIVE_PATH = (
    "scripts/campaigns/track1/exact_paper/"
    "run_track1_forward_last_four_open_cells_campaign.ps1"
)
...
LAUNCHER_NOTE_RELATIVE_PATH = (
    "doc/scripts/campaigns/run_track1_forward_last_four_open_cells_campaign.md"
)
CAMPAIGN_OUTPUT_DIRECTORY_TEMPLATE = (
    "output/training_campaigns/track1/exact_paper/forward_last_four_open_cells/{campaign_name}"
)
...
def build_attempt_specification_list(
    ...
    if repair_item.severity_label == "yellow_multi_surface":
...
def build_campaign_config(
    ...
    baseline_payload["experiment"]["run_name"] = (
        f"track1_forward_{family_slug}_{target_slug}_last_four_open_cells_{attempt_slug}"
    )
    baseline_payload["experiment"]["model_family"] = (
        "paper_reimplementation_rcim_original_dataset_exact_model_bank_forward_last_four_open_cells"
    )
    baseline_payload["experiment"]["model_type"] = (
        f"exact_model_bank_forward_last_four_open_cells_{family_slug}_{target_slug}"
    )
...
def build_campaign_directory_slug(repair_item: ForwardResidualRepairItem) -> str:
    ...
    return (
        ...
        f"h{repair_item.harmonic_order}_last_four_open_cells"
    )
...
def build_campaign_config_path(
    ...
    filename = (
        ...
        f"{repair_item.target_kind}_h{repair_item.harmonic_order}_last_four_open_cells_"
...
def main() -> None:
    ...
    active_campaign_payload = {
        ...
        "launch_command_list": [
            ".\\scripts\\campaigns\\track1\\exact_paper\\run_track1_forward_last_four_open_cells_campaign.ps1 -Remote",
        ],
        ...
        "interruption_note": (
            "Prepared as the final targeted forward-only residual repair wave "
            "after the completed forward maxi closeout."
        ),
    }
