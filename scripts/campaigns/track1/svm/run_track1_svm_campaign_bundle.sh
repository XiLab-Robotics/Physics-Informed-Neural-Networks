...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --campaign-config-file)
            ...
            ;;
        --help|-h)
            echo "Usage: bash scripts/campaigns/track1/svm/run_track1_svm_campaign_bundle.sh --campaign-config-root PATH --planning-report-path PATH --campaign-name NAME [--dry-run]"
            exit 0
            ;;
        *)
