...
export PINNS_PREPEND_NVIDIA_LD_LIBRARY_PATH="${NVIDIA_LIBRARY_PATH}"
export PINNS_PREVIOUS_LD_LIBRARY_PATH="\${LD_LIBRARY_PATH:-}"
if [ -n "\${LD_LIBRARY_PATH:-}" ]; then
    export LD_LIBRARY_PATH="\${PINNS_PREPEND_NVIDIA_LD_LIBRARY_PATH}:\${LD_LIBRARY_PATH}"
else
    export LD_LIBRARY_PATH="\${PINNS_PREPEND_NVIDIA_LD_LIBRARY_PATH}"
fi
EOF

cat >"${DEACTIVATE_SCRIPT_PATH}" <<'EOF'
if [ -n "${PINNS_PREVIOUS_LD_LIBRARY_PATH+x}" ]; then
    export LD_LIBRARY_PATH="${PINNS_PREVIOUS_LD_LIBRARY_PATH}"
else
    unset LD_LIBRARY_PATH
fi
unset PINNS_PREVIOUS_LD_LIBRARY_PATH
unset PINNS_PREPEND_NVIDIA_LD_LIBRARY_PATH
EOF
