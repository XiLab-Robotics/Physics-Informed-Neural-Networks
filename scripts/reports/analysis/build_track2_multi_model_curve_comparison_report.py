...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    argument_parser.add_argument(
        ...
        default=None,
        help="Dataset selector overriding the comparison YAML for this overlay report.",
    )
    argument_parser.add_argument(
        "--surface-scope",
        choices=["all", "forward", "backward", "global"],
        default="all",
        help="Limit the overlay report to one dataset-surface report scope.",
    )
    argument_parser.add_argument(
        "--candidate-id",
...
def filter_group_list_by_surface_scope(
    ...
    """Keep only overlay groups that can be built for one surface scope."""

    normalized_scope = normalize_surface_scope(surface_scope)
    filtered_group_list: list[ReportComparisonGroup] = []
    for group in group_list:
        if normalized_scope in {"forward", "backward"} and group.selection_mode != normalized_scope:
            continue
        if normalized_scope == "global" and group.selection_mode != "global":
            continue
        if all(candidate_id in candidate_lookup for candidate_id in group.candidate_id_list):
            filtered_group_list.append(group)
    assert filtered_group_list, f"No overlay report groups available for surface scope | {surface_scope}"
...
def resolve_report_candidate_configuration_list(
    ...
    if comparison_mode.startswith(("wave52r_full_candidate", "wave52r_integrated_specialist")):
...
def build_full_comparison_group_list(
    ...
    if (
        2 <= len(candidate_configuration_list) <= 6
        and len(candidate_source_label_set) >= 2
        and any(
            source_label.startswith("wave52r_")
            for source_label in candidate_source_label_set
        )
    ):
        group_list.append(
            ReportComparisonGroup(
                group_id="forward_wave52r_cross_lane_shortlist",
                group_title="Forward Wave 5.2R Cross-Lane Finalist Overlay",
                candidate_id_list=[
                    str(candidate_configuration["candidate_id"])
                    for candidate_configuration
                    in candidate_configuration_list
                ],
                selection_mode="forward",
            )
        )
...
def run_track2_multi_model_curve_comparison_report(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    assert str(arguments.surface_scope).strip().lower() != "global", (
        "The existing multi-model overlay builder supports forward/backward scopes. "
        "Use the dataset-surface matrix and collage reports for global scope."
    )
    ...
    if requested_candidate_id_set:
        ...
        assert resolved_candidate_id_set == requested_candidate_id_set, (
            "Could not resolve every requested overlay candidate | "
            f"missing={sorted(requested_candidate_id_set - resolved_candidate_id_set)}"
        )
    assert candidate_configuration_list, (
        "No overlay candidates available for requested surface scope | "
    ...
    group_list = filter_group_list_by_surface_scope(
        build_full_comparison_group_list(direction_metric_summary, candidate_configuration_list),
        candidate_lookup,
        arguments.surface_scope,
    )
    comparison_summary_list: list[dict[str, Any]] = []
