...
def build_directional_family_archive(
    ...
    reference_inventory_dictionary = {
        ...
        "canonical_selection_rule": (
            "Per direction and target, store the accepted canonical benchmark "
            "winner currently visible in Tables 2-5. When a later closeout "
...
def refresh_track1_reference_archives(
    accepted_target_artifact_map: dict[tuple[str, str, str], AcceptedTargetArtifact],
    direction_label_list: list[str] | None = None,
    archive_note_line: str = "This archive was rebuilt during the bidirectional original-dataset mega-campaign closeout.",
) -> list[dict[str, Any]]:

    """Rebuild the complete bidirectional `rcim_track1` archive surface."""

    if direction_label_list is None:
        direction_label_list = list(DIRECTION_ORDER)
    ...
    for direction_label in direction_label_list:
        for paper_family_code in FAMILY_ORDER:
