"""Close out the RCIM Model-Bank Reproduction forward paper-faithful grid-search campaign."""
...
def parse_command_line_arguments() -> argparse.Namespace:
    ...
    argument_parser = argparse.ArgumentParser(
        description="Close out the completed RCIM Model-Bank Reproduction forward paper-faithful grid-search campaign."
    )
...
def build_target_name(scope_key: str, harmonic_order: int) -> str:
    ...
    """Build one forward target name."""

    target_scope = "ampl" if scope_key == "amplitude" else "phase"
    return f"fft_y_Fw_filtered_{target_scope}_{int(harmonic_order)}"


def resolve_latest_forward_summary_dictionary() -> dict[str, dict[str, Any]]:

    """Resolve the latest completed forward summary for each archive family."""
    ...
    for summary_path in sorted(VALIDATION_ROOT.rglob("validation_summary.yaml")):
        ...
        if str(summary_dictionary["dataset"]["direction_label"]) != "forward":
            continue
        run_name = str(summary_dictionary["experiment"]["run_name"])
        if not run_name.startswith("track1_paper_faithful_grid_search_forward_"):
            continue
    ...
    assert not missing_family_list, (
        "Missing completed forward validation summaries | "
...
def resolve_family_export_entry(summary_dictionary: dict[str, Any]) -> dict[str, Any]:
    ...
    assert len(family_export_list) == 1, "Expected one family export entry per forward family run."
...
def build_accepted_artifact_map(
    ...
    """Build accepted target artifacts for all forward paper families plus ELM."""
    ...
    for paper_family, summary_bundle in summary_bundle_dictionary.items():
        ...
        for target_metric_entry in family_ranking_entry["target_metrics"]:
            ...
            assert direction_label == "forward", f"Expected forward target | {target_name}"
...
def refresh_forward_reference_archives(
    accepted_artifact_map: dict[tuple[str, str, str], archive_support.AcceptedTargetArtifact],
) -> list[dict[str, Any]]:

    """Refresh all forward family reference archives from the completed campaign."""
    ...
    for family_code in ARCHIVE_FAMILY_ORDER:
        archive_summary_list.append(
            archive_support.build_directional_family_archive(
                ...
                (
                    "This archive was replaced during the RCIM Model-Bank Reproduction forward "
...
def write_reference_root_readmes() -> None:
    ...
    """Refresh root archive README files without claiming backward ELM coverage."""
    ...
    backward_archive_line_list = [
        ...
        for family_code in PAPER_FAMILY_ORDER
    ]
    TRACK1_REFERENCE_ROOT_README_PATH.write_text(
        "\n".join([
            ...
            *backward_archive_line_list,
            ...
            "`ELM` is an additional operational RCIM Model-Bank Reproduction family and is archived for the",
    ...
    (TRACK1_REFERENCE_FORWARD_ROOT / "README.md").write_text(
        "\n".join([
    ...
    PAPER_REFERENCE_ROOT_README_PATH.write_text(
        "\n".join([
            ...
            "",
            "The RCIM Model-Bank Reproduction forward branch now includes the operational `ELM` archive in",
...
def update_forward_benchmark_tables(
    summary_bundle_dictionary: dict[str, dict[str, Any]],
    report_relative_path: str,
    finished_at: str,
) -> dict[str, dict[str, int]]:

    """Update forward Tables 2-5 in the RCIM benchmark report."""

    benchmark_line_list = BENCHMARK_REPORT_PATH.read_text(encoding="utf-8").splitlines()
    table_specification_list = [
        ("table2", "### Forward Table 2 - Amplitude MAE", "amplitude", "mae", AMPLITUDE_HARMONIC_LIST),
        ("table3", "### Forward Table 3 - Amplitude RMSE", "amplitude", "rmse", AMPLITUDE_HARMONIC_LIST),
        ("table4", "### Forward Table 4 - Phase MAE", "phase", "mae", PHASE_HARMONIC_LIST),
        ("table5", "### Forward Table 5 - Phase RMSE", "phase", "rmse", PHASE_HARMONIC_LIST),
    ]
    ...
    replacement_archive_status = [
        ...
        f"- RCIM Model-Bank Reproduction forward closeout report: `{report_relative_path}`",
        f"- RCIM Model-Bank Reproduction forward completion timestamp: `{finished_at}`",
...
def build_closeout_report(
    ...
    for table_key, table_label in (
        ("table2", "Forward Table 2 - Amplitude MAE"),
        ("table3", "Forward Table 3 - Amplitude RMSE"),
        ("table4", "Forward Table 4 - Phase MAE"),
        ("table5", "Forward Table 5 - Phase RMSE"),
    ):
    ...
    report_path.write_text(
        "\n".join([
            ...
            f"- refreshed archive root: `{format_project_relative_path(TRACK1_REFERENCE_FORWARD_ROOT)}`",
...
def patch_master_summary(report_relative_path: str, status_dictionary: dict[str, dict[str, int]]) -> None:
    ...
    replacement = "\n".join([
        ...
        f"- Table `2` `forward` status: `{status_dictionary['table2']['green']}` green, `{status_dictionary['table2']['yellow']}` yellow, `{status_dictionary['table2']['red']}` red",
        f"- Table `3` `forward` status: `{status_dictionary['table3']['green']}` green, `{status_dictionary['table3']['yellow']}` yellow, `{status_dictionary['table3']['red']}` red",
        f"- Table `4` `forward` status: `{status_dictionary['table4']['green']}` green, `{status_dictionary['table4']['yellow']}` yellow, `{status_dictionary['table4']['red']}` red",
        f"- Table `5` `forward` status: `{status_dictionary['table5']['green']}` green, `{status_dictionary['table5']['yellow']}` yellow, `{status_dictionary['table5']['red']}` red",
        "- Backward RCIM Model-Bank Reproduction Tables `2`-`5` remain pending until the matching backward paper-faithful campaign closes.",
    ...
    recent_campaign_row = (
        "| `track1_bidirectional_paper_faithful_grid_search_campaign_2026-05-04_12_26_30__forward_svr_mlp_rf_dt_et_ert_gbm_hgbm_lgbm_xgbm_elm_search` "
        "| `2026-05-15-07-07-30` | 11 | 0 | `forward` | Refreshed RCIM Model-Bank Reproduction forward paper-reference archives and RCIM Tables `2`-`5` |"
    )
...
def update_active_campaign_state(
    ...
    """Mark the forward campaign slice completed in active state."""
    ...
    active_campaign_dictionary["interruption_note"] = (
        "Forward paper-faithful grid-search slice completed and closed out. "
        "Backward paper-faithful slice remains a future launch."
    )
...
def main() -> None:
    ...
    """Run the RCIM Model-Bank Reproduction forward paper-faithful closeout."""
    ...
    archive_summary_list = refresh_forward_reference_archives(accepted_artifact_map)
    report_timestamp = "2026-05-15-11-11-35"
    report_path = REPORT_OUTPUT_ROOT / f"{report_timestamp}_track1_forward_paper_faithful_grid_search_closeout_report.md"
    report_relative_path = format_project_relative_path(report_path)
    status_dictionary = update_forward_benchmark_tables(
    ...
    print(f"[DONE] RCIM Model-Bank Reproduction forward paper-faithful closeout report | {report_path}")
