"""Close out the Track 1 forward paper-faithful grid-search campaign."""
...
def parse_command_line_arguments() -> argparse.Namespace:
    ...
    argument_parser = argparse.ArgumentParser(
        description="Close out the completed Track 1 forward paper-faithful grid-search campaign."
    )
    ...
    for family_code in ARCHIVE_FAMILY_ORDER:
        archive_summary_list.append(
            archive_support.build_directional_family_archive(
                ...
                (
                    "This archive was replaced during the Track 1 forward "
...
def update_forward_benchmark_tables(
    ...
    replacement_archive_status = [
        ...
        f"- Track 1 forward closeout report: `{report_relative_path}`",
        f"- Track 1 forward completion timestamp: `{finished_at}`",
...
def patch_master_summary(report_relative_path: str, status_dictionary: dict[str, dict[str, int]]) -> None:
    ...
    recent_campaign_row = (
        ...
        "| `2026-05-15-07-07-30` | 11 | 0 | `forward` | Refreshed Track 1 forward paper-reference archives and RCIM Tables `2`-`5` |"
    )
...
def main() -> None:
    ...
    print(f"[DONE] Track 1 forward paper-faithful closeout report | {report_path}")
