...
class MLModelMultipleOutput:
    ...
    def exportModel(self,modelName,colsToPredict):
        ...
        for i in range(len(self.model.estimators_)):
            ...
            # onx = to_onnx(
            #     est, target_opset={"": 12, "ai.onnx.ml": 2}
            # )
            #onx = convert_sklearn(est, initial_types=initial_type)
        ...
        if acronim == 'DT':
            parameters['DT'] = {
                ...
                #                               [self.model.estimator.get_params()['max_leaf_nodes']])),
                   'estimator__min_samples_split': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,4,12)) +
            ...
            parameters['ERT'] = {
                   'estimator__n_estimators':list(dict.fromkeys(list([20,40,60,80,100]) +
            ...
            parameters['RF'] = {
                   'estimator__n_estimators': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,50,100)) +
                                              [self.model.estimator.get_params()['n_estimators']])),
                   'estimator__criterion': list(dict.fromkeys(list(['squared_error', 'absolute_error']) +
                                              [self.model.estimator.get_params()['criterion']])),
                #    'estimator__max_features': list(dict.fromkeys(list(["log2","sqrt"]) +
                #                               [self.model.estimator.get_params()['max_features']])),
                   'estimator__max_depth': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,12,18)) +
                                              [self.model.estimator.get_params()['max_depth']])),
                   'estimator__min_samples_split': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,2,10)) +
                                              [self.model.estimator.get_params()['min_samples_split']])),
            }
            ...
            parameters['GBM'] = {
                'estimator__n_estimators': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,20,100)) +
                                              [self.model.estimator.get_params()['n_estimators']])),
                'estimator__criterion': list(dict.fromkeys(list(['squared_error']))),
                #                              [self.model.estimator.get_params()['criterion']])),
                # 'estimator__max_features': list(dict.fromkeys(list(["log2", "sqrt"]) +
                #                               [self.model.estimator.get_params()['max_features']])),
                'estimator__max_depth': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,14,21)) +
                                              [self.model.estimator.get_params()['max_depth']])),
                'estimator__min_samples_split': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,10,20)) +
                                              [self.model.estimator.get_params()['min_samples_split']])),
                'estimator__learning_rate':list(dict.fromkeys(list([0.001, 0.01, 0.1, 1.0]) +
                                              [self.model.estimator.get_params()['min_samples_split']])),
            }
        elif acronim == 'XGBM':
            parameters['XGBM'] = {
                ...
                #                               [self.model.estimator.get_params()['n_estimators']])),
                'estimator__max_depth': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(3,14,21)) +
                                              [self.model.estimator.get_params()['max_depth']])),
            ...
            parameters['HGBM'] = {
                # 'estimator__max_iter': list(dict.fromkeys(list([10]) +
                #                               [self.model.estimator.get_params()['max_iter']])),
                'estimator__max_iter': list(dict.fromkeys(list([])))+[self.model.estimator.get_params()['max_iter']],           
                'estimator__max_depth': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,9,14)) +
                                              [self.model.estimator.get_params()['max_depth']])),
                'estimator__learning_rate': list(dict.fromkeys(list([x/ 100 for x in self.genera_numeri_uniformi_interi(5,18,23)]) +
                                              [self.model.estimator.get_params()['learning_rate']])),
                'estimator__max_leaf_nodes':list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,24,29)) +
                                              [self.model.estimator.get_params()['max_leaf_nodes']])),
            }
        elif acronim == 'LGBM':
            parameters['LGBM'] = {
                'estimator__learning_rate': list(dict.fromkeys(list([x / 100 for x in self.genera_numeri_uniformi_interi(5, 1, 100)]) +
                                              [self.model.estimator.get_params()['learning_rate']])),
                 'estimator__max_depth': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5, 10, 16)) +
                                              [self.model.estimator.get_params()['max_depth']])),
                 'estimator__num_leaves': list(dict.fromkeys(list(self.genera_numeri_uniformi_interi(5,2,15)) +
                                              [self.model.estimator.get_params()['num_leaves']])),
                 'estimator__subsample': list(dict.fromkeys(list([0.001, 0.01, 0.1, 0.2]) +
            ...
            parameters['SVM'] = {
                     ...
                     'estimator__C':  list(dict.fromkeys(list([0.001,0.01,0.1,0.3, 1]) +
                                              [self.model.estimator.get_params()['C']])),
                     'estimator__epsilon': list(dict.fromkeys(list([0.001, 0.0001, 0.00001, 0.000001]))),
                    #  'estimator__gamma': list(dict.fromkeys(list([0.000011, 0.0000011, 0.0000022]))),
                    # 'estimator__epsilon': list(dict.fromkeys(list([ 0.00001]))),
    ...
    def predictorMLEvalutationOnTrain(self, dfInput, testSetDimension):
        ...
        # Prende solo una parte del train
        #X_train = X_train.iloc[:410]
        #Y_train = Y_train.iloc[:410]


        #Trucco SVR

        #X_train = pd.concat([X_test,X_test]) 
        #Y_train = pd.concat([Y_test,Y_test]) 

        # X_train = X_train.iloc[:-len(X_train)]
        # Y_train = Y_train.iloc[:-len(Y_train)]
        # X_train = pd.concat([X_train,X_test.iloc[:20]])
        # Y_train = pd.concat([Y_train,Y_test.iloc[:20]])
        # X_train = pd.concat([X_train,X_test.iloc[:20]])
        # Y_train = pd.concat([Y_train,Y_test.iloc[:20]])
        ...
        ## save scores for each model and for the aggregated
        # outputFileSummary = 'output_prediction/summaryEvalutationOnTrain+_' + self.name.split('_')[-2:][0] \
        #                     + '_' + self.name.split('_')[-2:][1] + '.csv'
        outputFileSummary = 'output_prediction/summaryEvalutationOnTrain+_' + self.name.split('_')[-2:][1] + '.csv'
        finalOut = pd.DataFrame(crossValOut, index=[0])
        if os.path.isfile(outputFileSummary):
            # Se il file esiste già, carica il contenuto esistente in un DataFrame
            existing_df = pd.read_csv(outputFileSummary, sep=';', decimal=',')
    ...
    def predictorMLCrossValidationWithHyperparameter(self, dfInput,testSetDimension):
        ...
        logFile = 'logGSCV_'+self.name.split('_')[-2:][0]\
                            +'_'+self.name.split('_')[-2:][1]+'_20240112.txt'

        with open(logFile, 'a') as f:
            f.write("MODEL:"+self.name+'\n')
            f.write("TRAINING START:"+str(datetime.datetime.now())+'\n')
            f.write(str(self.model.param_grid)+'\n')
        ...
        with open(logFile, 'a') as f:
            f.write("TRAINING END:"+str(datetime.datetime.now())+'\n')
            f.write(str(self.model.best_params_)+'\n')
