...
- In styled PDF tables, when a narrow metric header includes a compact unit such
  as `[deg]`, `[rpm]`, or a similar measurement suffix, prefer rendering the
  metric name on the first line and the unit on a second line inside the same
  header cell instead of over-compressing the column or forcing the full header
  onto one line.
...
- Before the final commit, when approved work adds or materially changes a script, feature, runnable workflow, documentation entry point, or other user-facing repository behavior, update the canonical Sphinx source tree under `site/` as needed so the portal remains aligned with the real repository state.
- Before the final commit, whenever approved work affects the canonical Sphinx portal scope, regenerate the portal with `python -m sphinx -W -b html site site/_build/html` and treat a warning-free build as part of the task completion bar.
- Treat GitHub Pages publication through the repository-owned GitHub Actions workflow as the canonical hosting path for the Sphinx portal. If the workflow is changed or the published entry points move, update the relevant local-build and publication documentation before closing the task.
